...
app.get('/apple-touch-icon.png', (req, res) => {
    ...
    else res.status(404).send('Icon not uploaded yet.');
});
        ...
        db.get(`SELECT id FROM users WHERE youth_id = ? OR username = ?`, [youthId, targetQr], (err2, existingUser) => {
            if (existingUser) {
                ...
                db.run(`INSERT INTO users (username, password, permissions, youth_id, created_at) VALUES (?, ?, ?, ?, ?)`,
                    [targetQr, targetQr, permString, youthId, getManilaTime()], function(err4) {
                        if (err4) {
...
app.get('/api/users/list', (req, res) => {
    ...
    db.all(sql, [], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(rows.map(r => ({
            id: r.id, username: r.username, display_name: r.member_name ? `${r.member_name}` : r.username,
            qr_code: r.member_code || r.username, youth_id: r.youth_id, permissions: r.permissions || '[]'
        })));
    });
});
...
app.put('/api/attendance/:id', (req, res) => {
    ...
    db.run(`UPDATE attendance SET checked_in_at = ?, is_walkin = ? WHERE id = ?`, [checked_in_at, is_walkin ? 1 : 0, req.params.id], function (err) {
        ...
        db.get(`SELECT COUNT(*) as total_youth FROM youth WHERE age IS NOT NULL AND age != ''`, [], (err2, totalYouthRow) => {
            ...
            db.all(sqlRoster, [eventId], (err3, roster) => {
                ...
                db.all(sqlPreReg, [eventId], (err4, preRegList) => {
                    ...
                    if (totalPreRegistered > 0) turnoutPercentage = ((checkedInPreRegs / totalPreRegistered) * 100).toFixed(1);
                    res.json({ event, totalDirectory, totalTurnout, turnoutPercentage, walkins, preReg: checkedInPreRegs, totalPreRegistered, roster, preRegList });
                });
            });
        });
    });
    ...
    db.get(`SELECT poster, prereg_banner FROM events WHERE id = ?`, [req.params.id], (err, event) => {
        ...
        if (base64String && base64String.startsWith('data:image')) {
            try {
                ...
                res.writeHead(200, { 'Content-Type': mimeData, 'Content-Length': imgBuffer.length });
                res.end(imgBuffer);
            } catch (error) { res.status(500).send('Error processing image'); }
        } else res.status(404).send('No image uploaded');
    });
});
...
app.post('/api/events', (req, res) => {
    ...
    db.run(`INSERT INTO events (name, event_date, time_start, venue, poster, photos_url, materials_url, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [name, event_date, time_start, venue, poster, photos_url, materials_url, getManilaTime()], function (err) {
...
app.put('/api/events/:id', (req, res) => {
    ...
    if (poster !== undefined && poster !== null) {
        db.run(`UPDATE events SET name=?, event_date=?, time_start=?, venue=?, poster=?, photos_url=?, materials_url=? WHERE id=?`,
            [name, event_date, time_start, venue, poster, photos_url, materials_url, req.params.id], function (err) {
        ...
        db.run(`UPDATE events SET name=?, event_date=?, time_start=?, venue=?, photos_url=?, materials_url=? WHERE id=?`,
            [name, event_date, time_start, venue, photos_url, materials_url, req.params.id], function (err) {
...
app.post('/api/discipleship/milestones', (req, res) => {
    ...
    db.run(`INSERT INTO member_milestones (youth_id, pathway_id, status, completed_at, notes) VALUES (?, ?, ?, ?, ?) ON CONFLICT(youth_id, pathway_id) DO UPDATE SET status = excluded.status, completed_at = excluded.completed_at, notes = excluded.notes`,
        [youth_id, pathway_id, status, status === 'Completed' ? getManilaTime() : null, notes], function(err) {
            logActivity(actor, 'UPDATE_MILESTONE', `Updated milestone ID ${pathway_id} for youth ID ${youth_id}`);
            res.json({ success: true });
        });
});
...
app.post('/api/discipleship/pathways', (req, res) => { const { title, description, step_order, actor } = req.body; db.run(`INSERT INTO discipleship_pathways (title, description, step_order, created_at) VALUES (?, ?, ?, ?)`, [title, description, step_order, getManilaTime()], function(err) { res.json({ success: true, id: this.lastID }); }); });
app.put('/api/discipleship/pathways/:id', (req, res) => { const { title, description, step_order } = req.body; db.run(`UPDATE discipleship_pathways SET title=?, description=?, step_order=? WHERE id=?`, [title, description, step_order, req.params.id], function(err) { res.json({ success: true }); }); });
...
app.get('/api/discipleship/member-progress/:youth_id', (req, res) => {
    const youthId = req.params.youth_id;
    db.all(`SELECT p.id as pathway_id, p.title, m.status, m.completed_at, m.notes as pastoral_notes FROM discipleship_pathways p LEFT JOIN member_milestones m ON p.id = m.pathway_id AND m.youth_id = ? ORDER BY p.step_order ASC`, [youthId], (err, rows) => { res.json(rows); });
});

// ADMIN ANALYTICS - CONGREGATION SPIRITUAL STAGES
app.get('/api/discipleship/analytics/stages', (req, res) => {
    const sql = `
        WITH UserMaxStep AS (
            SELECT youth_id, MAX(pathway_id) as max_path_id
            FROM member_milestones
            WHERE status = 'Completed' OR status = 'In Progress'
            GROUP BY youth_id
        )
        SELECT p.title, COUNT(u.youth_id) as user_count
        FROM discipleship_pathways p
        LEFT JOIN UserMaxStep u ON p.id = u.max_path_id
        GROUP BY p.id, p.title
        ORDER BY p.step_order ASC
    `;
    db.all(sql, [], (err, stepRows) => {
        if (err) return res.status(500).json({ error: err.message });
        db.get(`SELECT COUNT(*) as total FROM youth`, [], (err, youthRow) => {
            const totalYouth = youthRow ? youthRow.total : 0;
            let assignedYouth = 0;
            stepRows.forEach(r => assignedYouth += r.user_count);
            const unassigned = totalYouth - assignedYouth;
            res.json({ stages: stepRows, unassigned: unassigned > 0 ? unassigned : 0 });
        });
    });
});
...
app.post('/api/journals', (req, res) => { const { youth_id, title, content, mood } = req.body; db.run(`INSERT INTO private_journals (youth_id, title, content, mood, created_at) VALUES (?, ?, ?, ?, ?)`, [youth_id, title, content, mood, getManilaTime()], function(err) { res.json({ success: true }); }); });
...
app.post('/api/prayers', (req, res) => { const { youth_id, title, request, is_anonymous } = req.body; db.run(`INSERT INTO prayer_requests (youth_id, title, request, is_anonymous, created_at) VALUES (?, ?, ?, ?, ?)`, [youth_id, title, request, is_anonymous ? 1 : 0, getManilaTime()], function(err) { res.json({ success: true }); }); });
...
app.post('/api/small-groups', (req, res) => { const { name, leader_id, meeting_schedule, venue } = req.body; db.run(`INSERT INTO small_groups (name, leader_id, meeting_schedule, venue, created_at) VALUES (?, ?, ?, ?, ?)`, [name, leader_id || null, meeting_schedule, venue, getManilaTime()], function(err) { res.json({ success: true }); }); });
app.put('/api/small-groups/:id', (req, res) => { const { name, leader_id, meeting_schedule, venue } = req.body; db.run(`UPDATE small_groups SET name=?, leader_id=?, meeting_schedule=?, venue=? WHERE id=?`, [name, leader_id || null, meeting_schedule, venue, req.params.id], function(err) { res.json({ success: true }); }); });
...
app.post('/api/ai/chat', (req, res) => {
    ...
    logActivity(actor || 'System', 'AI_QUERY', `User asked AI: "${prompt}"`);

    if (q.includes('missing') || q.includes('absent') || q.includes('not attend') || q.includes('hasn\'t attended')) {
        const sql = `SELECT y.name, MAX(a.checked_in_at) as last_seen FROM youth y LEFT JOIN attendance a ON y.id = a.youth_id GROUP BY y.id ORDER BY last_seen ASC LIMIT 10`;
        db.all(sql, [], (err, rows) => {
            if (err) return res.json({ response: "I encountered a database error looking up attendance." });
            let msg = "Here are the community members who haven't checked into an event recently (or ever):<br><br>";
            rows.forEach(r => { msg += `• <strong>${r.name}</strong> - Last seen: ${r.last_seen ? r.last_seen : '<em>Never checked in</em>'}<br>`; });
            msg += "<br>Would you like to send them a follow-up message?";
            res.json({ response: msg });
        });
        return;
    }

    if (q.includes('how many member') || q.includes('total member') || q.includes('directory size')) {
        db.get(`SELECT count(*) as total FROM youth`, [], (err, row) => {
            let msg = `Praise God! We currently have <strong>${row.total} members</strong> registered in the FOG Community Directory.`;
            res.json({ response: msg });
        });
        return;
    }

    if (q.includes('pray') || q.includes('prayer request')) {
        db.all(`SELECT title, request FROM prayer_requests WHERE status = 'Open' LIMIT 5`, [], (err, rows) => {
            if (rows.length === 0) return res.json({ response: "There are currently no open prayer requests on the wall." });
            let msg = "Here are some recent prayer requests from the community that need intercession:<br><br>";
            rows.forEach(r => { msg += `🙏 <strong>${r.title}</strong>: "${r.request}"<br><br>`; });
            res.json({ response: msg });
        });
        return;
    }

    if (q.includes('small group') || q.includes('connect group')) {
        db.all(`SELECT name, meeting_schedule FROM small_groups LIMIT 5`, [], (err, rows) => {
            if (rows.length === 0) return res.json({ response: "There are no small groups registered yet." });
            let msg = "Here are the active Small Groups currently meeting:<br><br>";
            rows.forEach(r => { msg += `• <strong>${r.name}</strong> - Meets: ${r.meeting_schedule || 'TBA'}<br>`; });
            res.json({ response: msg });
        });
        return;
    }
    ...
    setTimeout(() => {
        res.json({
            response: `Hello! I am your <strong>FOG Ministry AI Assistant</strong>.<br><br>Since I am currently running in MVP (Local Mode) to save costs, I can answer specific questions about our database. Try asking me:<br><br>• <em>"Who hasn't attended recently?"</em><br>• <em>"How many members do we have?"</em><br>• <em>"Are there any prayer requests?"</em><br>• <em>"Show me the small groups."</em>`
        });
    }, 600);
});
