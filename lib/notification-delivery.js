...
function createWebPushSender({
    ...
    if (
        ...
        throw new TypeError(
            'webpush.sendNotification is required when Push is enabled'
        );
    }

    return async function sendCanonicalPush({
        subscription,
        payload
    } = {}) {
        if (
        ...
        /*
         * web-push rejects with statusCode on provider
         * failure. We deliberately allow that exception
         * to propagate so the canonical delivery engine
         * can classify 404/410, transient 5xx, etc.
         */
        const response =
        ...
        if (
            response &&
            response.headers &&
            typeof response.headers.location ===
                'string' &&
...
function createRuntimeNotificationDeliveryEngine({
    database,
    webpush,
    pushEnabled,
    emailOutbox = null,
    ...
    return createNotificationDeliveryEngine({
        database,
        sendPush,
        emailOutbox:
            emailOutbox || null,
