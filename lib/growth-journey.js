...
async function recalculatePhaseProgress(db, youthId, phaseId, {
    ...
    /*
     * A completed phase is a durable standing, not a volatile percentage.
     * In particular, a legacy transition deliberately records standing
     * without manufacturing modern task evidence. Preserve the strongest
     * previously recorded percentage while continuing to expose the real
     * per-task evidence calculation below.
     */
...
async function ensureOnboardingPrayerPartner(db, youthId, {
    ...
    if (
        !Number.isInteger(normalizedYouthId) ||
        normalizedYouthId <= 0
    ) {
        throw new Error('youthId is required');
    }
    ...
    /*
     * Prayer Partner rotation is Monday-keyed.
     *
     * On-demand newcomer assignment must use the same Manila Monday
     * key so it belongs to the current weekly snapshot rather than
     * creating a separate calendar-day bucket.
     */
    const canonicalWeek =
        getManilaMondayKey(
            assignedAt || new Date()
    ...
    /*
     * Historical assignments do not satisfy a new week.
     * Only a row for the current canonical week is authoritative.
     */
    const existing = await get(
        db,
        `
    ...
    /*
     * Prefer somebody already participating in this week's Prayer
     * Partner snapshot.
     *
     * Multiple senders may intentionally pray for the same recipient
     * when a newcomer is added after the normal weekly rotation.
     * The selected recipient's own assignment is never changed.
     */
    let candidate = await get(
        ...
        WHERE y.id <> ?
          AND EXISTS (
                SELECT 1
                FROM secret_prayer_pals current
                WHERE current.week_start = ?
                  AND (
                        current.youth_id = y.id
                        OR current.pal_youth_id = y.id
                  )
          )
        ORDER BY RANDOM()
        LIMIT 1
    ...
    /*
     * Safe fallback when the current week's normal snapshot does not
     * yet exist.
     */
    if (!candidate) {
        candidate = await get(
    ...
    /*
     * Final small-community fallback.
     */
    if (!candidate) {
        candidate = await get(
            db,
    ...
    if (
        ...
        candidateId <= 0 ||
        candidateId === normalizedYouthId
    ) {
        throw new Error(
            'Invalid Prayer Partner candidate'
        );
    }

    /*
     * UNIQUE(youth_id, week_start) makes this concurrency-safe.
     * If two simultaneous requests race, one INSERT wins and the
     * other request re-reads the winning current-week assignment.
     */
    const inserted = await run(
        db,
        `
    ...
    if (
        Number(assignment.youth_id) ===
        Number(assignment.pal_youth_id)
    ) {
        throw new Error(
            'Prayer Partner cannot be self'
        );
    }
...
async function getMemberJourney(db, youthId, {
    ...
    for (const phase of phases) {
        ...
        journey.push({
            ...
            completionBasis: progress.completionBasis,
...
function isCompletePrayerPartnerSnapshot(
    ...
    for (const row of rows) {
        ...
        /*
         * Duplicate RECIPIENTS are allowed.
         *
         * This is intentional for members who join after the normal
         * Monday rotation. Every sender still has exactly one current
         * assignment, but more than one sender may pray for the same
         * recipient.
         */
        senders.add(youthId);
    }
