...
async function getNextEligibleCampaign(db, youthId, {
    now,
    campaignFilter = null
} = {}) {
    ...
    if (
        campaignFilter !== null &&
        typeof campaignFilter !== 'function'
    ) {
        throw new TypeError(
            'campaignFilter must be a function when provided'
        );
    }
    ...
    for (const campaign of campaigns) {
        ...
        if (
            campaignFilter &&
            !(await campaignFilter(campaign, member))
        ) {
            continue;
        }
