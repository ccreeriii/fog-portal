...
/*
 * Age is derived data whenever a valid birthday exists.
 *
 * This helper is used by profile-update paths so changing
 * a birthday does not have to wait until the next midnight
 * birthday scheduler run.
 */
function deriveAgeFromBirthday(
    birthday,
    now = new Date()
) {
    const dateKey =
