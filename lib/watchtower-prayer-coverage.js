...
function normalizeConfig({
    ...
    return {
        openHour:
            normalizedOpen < normalizedReport
                ? normalizedOpen
                : DEFAULT_OPEN_HOUR,
        reportHour:
            normalizedOpen < normalizedReport
...
function getWindowStatus(
    ...
    const phase =
        clock.hour < normalized.openHour
            ? 'before_open'
            : clock.hour < normalized.reportHour
                ? 'open'
                : 'closed';
    ...
    return {
        ...
        closes_at:
            `${clock.dateKey}T${String(normalized.reportHour).padStart(2, '0')}:00:00+08:00`
    };
}
...
async function createDailyCoverageReport(
    ...
    if (clock.hour < config.reportHour) {
        return {
            status:
                'not_due',
            manila_date:
            ...
            notification =
                await notificationCenter
                    .createNotification(
                        ...
                        {
                            eventKey:
                                `watchtower-coverage-report:date:${clock.dateKey}`,
                            category:
                                'prayer_daily_growth',
                            title:
                                'Watchtower Coverage Report',
                            message:
                                `${report.total_covered} of ${report.eligible_population} participating members received Prayer Coverage today (${report.coverage_percent}%).`,
                            importance:
                                report.uncovered > 0
                                    ? 'important'
                                    : 'normal',
