...
async function createIntake(db, {
    ...
    /*
     * A member must have only one transition pathway
     * within a transition version.
     *
     * The check and possible insert share one IMMEDIATE
     * transaction so two different pathway requests cannot
     * legitimately create two transition kinds.
     */
    return withImmediateTransaction(
        ...
        async () => {
            ...
            if (conflicting) {
                throw transitionError(
                    'TRANSITION_KIND_CONFLICT',
                    'This member already has a different transition pathway for this transition version.'
                );
            }

            /*
             * Same-kind retries remain deliberately
             * idempotent.
             */
            const sameKind =
                existing.find(
...
async function canonicalizeReportedMinistries(
    db,
    reportedMinistries
) {
    ...
    for (const item of reportedMinistries) {
        if (
            !item ||
            typeof item !== 'object'
        ) {
        ...
        if (!ministryId) {
            throw transitionError(
                'CANONICAL_MINISTRY_REQUIRED',
                'Choose a ministry from the FOG ministry list.'
            );
        }
        ...
        if (
            !stored ||
            !normalizeId(stored.id) ||
            typeof stored.name !== 'string' ||
            !stored.name.trim()
        ) {
            throw transitionError(
                'CANONICAL_MINISTRY_NOT_FOUND',
                'The selected ministry is no longer available.'
            );
        }

        /*
         * Canonical ministry identity and name are server-owned.
         * The browser-supplied ministry name is deliberately ignored.
         */
        canonical.push(
...
async function saveDraftIntake(db, {
    ...
    const ministries =
        await canonicalizeReportedMinistries(
            db,
            reportedMinistries
        );
