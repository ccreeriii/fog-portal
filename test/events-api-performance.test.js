...
test('event frontend consumes list references, selected detail, and deduplicates list loads', async () => {
    ...
    assert.ok(serviceWorkerSource.includes("const CACHE_NAME = 'fog-portal-v62';"), 'new app asset has a fresh shell cache');
    ...
    assert.ok(serviceWorkerSource.includes("'/js/app.js?v=13.5'"), 'service worker caches the coordinated app version');
    assert.ok(serviceWorkerSource.includes("'/js/v10-expansion.js?v=12.3'"), 'service worker caches the Arcade fix');
    assert.ok(indexSource.includes('<script src="/js/app.js?v=13.5"></script>'), 'index serves the coordinated app version');
