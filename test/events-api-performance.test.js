...
test('events API uses lightweight list, selected detail, and allowlisted media', { concurrency: false }, async (t) => {
    ...
    // Copying the complete lib tree avoids fragile dependency whitelists
    // as production modules evolve.
    await fsp.cp(
...
test('event frontend consumes list references, selected detail, and deduplicates list loads', async () => {
    ...
    assert.ok(serviceWorkerSource.includes("const CACHE_NAME = 'fog-portal-v84';"), 'new app asset has a fresh shell cache');
