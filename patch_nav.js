...
const newBuildNav = `window.buildNav = function() {
    ...
    // 🛡️ THE ENFORCER: Instantly cleans up any rogue button injections from V3/V10
    setTimeout(() => {
        if (bottomNav) {
            const allowed = ['Profile', 'Inbox', 'Growth', 'Milestone', 'Journal', 'Prayer', 'Groups'];
            Array.from(bottomNav.children).forEach(child => {
                const span = child.querySelector('span');
                if (span && !allowed.includes(span.innerText.trim())) {
                    child.remove();
                }
            });
        }
...
if (startIdx !== -1 && endIdx !== -1) {
    fs.writeFileSync('public/js/app.js', appJs.substring(0, startIdx) + newBuildNav + appJs.substring(endIdx));
    console.log('✅ Navigation successfully overhauled!');
} else {
    console.log('❌ Could not find window.buildNav bounds');
}
