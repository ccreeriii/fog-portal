...
(function(global) {
  ...
  // Load or reference KoinoniaGrowth if available (Phase 0.23J)
  ...
  function setIdentity(personaKey) {
    ...
    let key = String(personaKey).toUpperCase();
    if (!PERSONAS[key]) {
      const foundEntry = Object.entries(PERSONAS).find(([k, p]) =>
        p.id === personaKey || p.memberId === personaKey || k.toLowerCase() === String(personaKey).toLowerCase()
      );
      if (foundEntry) {
        key = foundEntry[0];
      } else {
  ...
  function getGrowthProfile(identity) {
    const id = identity || getCurrentIdentity();
    if (KoinoniaGrowth && typeof KoinoniaGrowth.getGrowthProfile === 'function') {
      return KoinoniaGrowth.getGrowthProfile(id);
    }
    return null;
  }
