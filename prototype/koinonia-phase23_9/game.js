...
(function () {
  ...
  function getQuest(questId) {
    ...
    if (typeof KoinoniaContentRegistry !== 'undefined' && typeof KoinoniaContentRegistry.getQuestById === 'function') {
      const regQuest = KoinoniaContentRegistry.getQuestById(questId);
      if (regQuest) return regQuest;
    }
    return QUESTS.find(q => q.id === questId || String(q.id).toUpperCase() === String(questId).toUpperCase()) || null;
  }

  function evaluateQuestAvailability() {
    const allQuests = (typeof KoinoniaContentRegistry !== 'undefined' && typeof KoinoniaContentRegistry.getPublishedQuests === 'function')
      ? KoinoniaContentRegistry.getPublishedQuests({ includeInactive: true })
      : QUESTS;
  ...
  function grantQuestRewards(questId) {
    ...
    const activeMemberId = (typeof BetaIdentityProvider !== 'undefined' && typeof BetaIdentityProvider.getCurrentIdentity === 'function')
      ? (BetaIdentityProvider.getCurrentIdentity()?.memberId || 'youth_demo_01')
      : 'youth_demo_01';

    let authorityResult = null;
    if (typeof KoinoniaRewardAuthority !== 'undefined' && typeof KoinoniaRewardAuthority.completeQuest === 'function') {
      authorityResult = KoinoniaRewardAuthority.completeQuest(activeMemberId, questId);
    }

    if (authorityResult) {
      if (!authorityResult.success) {
        console.error(`Reward authority rejected quest ${questId}:`, authorityResult.error);
        return;
      }
      if (authorityResult.alreadyCompleted) {
        console.warn(`Reward authority reported quest ${questId} was already completed. Skipping duplicate grant.`);
    ...
    // 1. Reward delta metadata (preserved for messages, toasts, animations, analytics)
    const lpVal = authorityResult ? authorityResult.rewardsGranted.lifePoints :
                  ((typeof quest.rewards.lp === 'number') ? quest.rewards.lp :
                   (typeof quest.rewards.lifePoints === 'number') ? quest.rewards.lifePoints : 0);
    const xpVal = authorityResult ? authorityResult.rewardsGranted.xp :
                  ((typeof quest.rewards.charXp === 'number') ? quest.rewards.charXp :
                   (typeof quest.rewards.characterXp === 'number') ? quest.rewards.characterXp :
                   (typeof quest.rewards.xp === 'number') ? quest.rewards.xp : 0);

    // 2. Absolute hydration from KoinoniaGrowth read model (zero UI delta arithmetic)
    const profile = (typeof KoinoniaGrowth !== 'undefined' && typeof KoinoniaGrowth.getGrowthProfile === 'function')
      ? KoinoniaGrowth.getGrowthProfile(activeMemberId)
      : null;
    ...
    } else if (typeof KoinoniaRewardAuthority !== 'undefined' && typeof KoinoniaRewardAuthority.getRewardSummary === 'function') {
      const summary = KoinoniaRewardAuthority.getRewardSummary(activeMemberId);
      if (summary) {
        state.lp = summary.lifePoints;
        state.charXp = summary.xp;
      }
    }
    ...
    if (state.questProgress[questId]) {
      ...
      if (authorityResult && authorityResult.completionId) {
        state.questProgress[questId].completionId = authorityResult.completionId;
      }
    }
    if (questId === 'Q-001') {
      state.questStatus = 'completed';
    ...
    if (authorityResult && Array.isArray(authorityResult.rewardsGranted.milestoneIds)) {
      authorityResult.rewardsGranted.milestoneIds.forEach(mId => {
        const cat = (typeof KoinoniaGrowth !== 'undefined' && KoinoniaGrowth.MILESTONE_CATALOG)
          ? KoinoniaGrowth.MILESTONE_CATALOG[mId]
          : null;
        const title = cat ? cat.title : mId;
        showToast(`🏆 Milestone Unlocked: ${title}`);
      });
    }
  ...
  function renderMilestones() {
    ...
    if (typeof KoinoniaGrowth !== 'undefined' && typeof KoinoniaGrowth.getGrowthProfile === 'function') {
      const activeId = (typeof BetaIdentityProvider !== 'undefined') ? BetaIdentityProvider.getCurrentIdentity() : null;
      growthProfile = KoinoniaGrowth.getGrowthProfile(activeId);
    }

    // Combine canonical milestone catalog with progression catalog
    const growthCatalog = (typeof KoinoniaGrowth !== 'undefined' && KoinoniaGrowth.MILESTONE_CATALOG)
      ? Object.values(KoinoniaGrowth.MILESTONE_CATALOG)
      : [];

    const combinedCatalog = [...growthCatalog];
    milestoneCatalog.forEach(m => {
      if (!combinedCatalog.some(c => c.id === m.id)) {
        combinedCatalog.push(m);
      }
    });
    ...
    if (growthProfile && Array.isArray(growthProfile.milestones)) {
      growthProfile.milestones.forEach(m => {
        achievedMap.set(m.id, m.achievedAt || true);
      });
    }

    let html = '';
    combinedCatalog.forEach(m => {
      const isAchievedInProfile = achievedMap.has(m.id);
      ...
      const unlockedDate = (isAchievedInProfile && achievedMap.get(m.id) !== true)
        ? achievedMap.get(m.id)
        : (state.milestones && state.milestones[m.id]?.unlockedAt);
  ...
  function renderJourneySummary() {
    ...
    const lvl = growthProfile ? growthProfile.progression.level : (state.charLevel || 1);
    const levelTitle = growthProfile ? growthProfile.progression.levelTitle : ((pData && pData.getLevelDef) ? pData.getLevelDef(lvl).title : 'New Explorer');
    const xp = growthProfile ? growthProfile.progression.xp : (state.charXp || 0);
    const lp = growthProfile ? growthProfile.progression.lifePoints : (state.lp || 0);
    const completedCount = growthProfile ? growthProfile.activity.questsCompleted : Object.values(state.questProgress || {}).filter(p => p.status === 'COMPLETED').length;
    const streakDays = growthProfile ? growthProfile.activity.currentQuestStreak : (state.streakDays || 0);
    const serviceActivities = growthProfile ? growthProfile.activity.serviceActivities : 0;
    const unlockedMilestonesCount = (growthProfile && Array.isArray(growthProfile.milestones))
      ? growthProfile.milestones.length
      : (Array.isArray(state.unlockedMilestones) ? state.unlockedMilestones.length : 0);

    const growthCatalog = (typeof KoinoniaGrowth !== 'undefined' && KoinoniaGrowth.MILESTONE_CATALOG)
      ? Object.keys(KoinoniaGrowth.MILESTONE_CATALOG)
      : [];
    const milestoneCatalog = (pData && (pData.MILESTONES || pData.milestones)) || [];
    const totalMilestonesCount = Math.max(unlockedMilestonesCount, growthCatalog.length || milestoneCatalog.length || 6);
    ...
    container.innerHTML = `
      <div class="summary-metric-card">
        ...
        <div class="summary-metric-sub">${levelTitle}</div>
      </div>
      <div class="summary-metric-card">
      ...
      <div class="summary-metric-card">
        ...
        <div class="summary-metric-value" style="color: var(--accent-success);">${lp} LP</div>
        <div class="summary-metric-sub">Participation Currency</div>
      </div>
      <div class="summary-metric-card">
        <div class="summary-metric-label">QUESTS COMPLETED</div>
      ...
      <div class="summary-metric-card">
        <div class="summary-metric-label">QUEST STREAK</div>
        <div class="summary-metric-value" style="color: var(--brand-fire-orange);">${streakDays} Days</div>
        <div class="summary-metric-sub">Daily Diligence</div>
      </div>
      <div class="summary-metric-card">
        <div class="summary-metric-label">SERVICE ACTIVITIES</div>
        <div class="summary-metric-value" style="color: var(--brand-burgundy);">${serviceActivities}</div>
        <div class="summary-metric-sub">Ministry & Service</div>
      </div>
      <div class="summary-metric-card">
  ...
  function showToast(msg) {
    ...
    toast.className = 'proximity-prompt koinonia-toast';
  ...
  function loadFromStorage() {
    ...
    try {
      ...
      // Restore progression values (In Phase 0.23J, canonical persona growth profile is authoritative)
      let activeGrowth = null;
      if (typeof KoinoniaGrowth !== 'undefined') {
        const curIdentity = (typeof BetaIdentityProvider !== 'undefined') ? BetaIdentityProvider.getCurrentIdentity() : null;
        activeGrowth = KoinoniaGrowth.getGrowthProfile(curIdentity);
      }
      if (activeGrowth && activeGrowth.progression) {
        state.lp = activeGrowth.progression.lifePoints;
        state.charLevel = activeGrowth.progression.level;
        state.charXp = activeGrowth.progression.xp;
        state.charXpMax = activeGrowth.progression.nextLevelXp;
        state.highestLevelReached = Math.max(state.charLevel, (typeof parsed.highestLevelReached === 'number' ? parsed.highestLevelReached : state.charLevel));
      } else {
  ...
  function openQuestsTabModal() {
    ...
    if (list) {
      ...
      const allQuests = (typeof KoinoniaContentRegistry !== 'undefined' && typeof KoinoniaContentRegistry.getPublishedQuests === 'function')
        ? KoinoniaContentRegistry.getPublishedQuests()
        : QUESTS;
      ...
      if (activeQuests.length > 0) {
        activeQuests.forEach(({ quest: q, prog }) => {
          ...
          const isStudio = (q.source === 'studio' || String(q.id).startsWith('studio:'));
          html += `
            <div class="team-chip" style="padding: 12px; margin-bottom: 6px; cursor: pointer;" onclick="window.KOINONIA_GAME.openQuestDetailModal('${q.id}')">
              <div style="flex: 1;">
                <div style="display: flex; align-items: center; gap: 6px;">
                  ...
                  ${isStudio ? '<span style="font-size: 0.60rem; background: #EEF2FF; color: #4338CA; padding: 1px 5px; border-radius: 3px; font-weight: 800;">STUDIO</span>' : ''}
                </div>
          ...
          html += `
            <div class="team-chip" style="padding: 12px; margin-bottom: 6px; cursor: pointer;" onclick="window.KOINONIA_GAME.openQuestDetailModal('${q.id}')">
              <div style="flex: 1;">
                ...
                <div style="font-size: 0.68rem; color: var(--brand-fire-orange); font-weight: 700;">${q.giverName ? `Offered by ${q.giverName}` : (isStudio ? 'Community Mission' : 'Offered by Elder')} • +${q.rewards.lp} LP</div>
              </div>
  ...
  function getSharedCore() {
    ...
    if (typeof global !== 'undefined') {
      if (typeof global.getSharedCoreProvider === 'function') return global.getSharedCoreProvider();
      if (global.SharedCore) return global.SharedCore;
    }
  ...
  function isAuthorizedAdmin() {
    const rc = (typeof window !== 'undefined') ? window.__KOINONIA_RUNTIME_CONFIG__ : null;
    const sc = getSharedCore();
    const isRemoteMode = Boolean(
      (rc && (rc.sharedCoreMode === 'mock' || rc.sharedCoreMode === 'unavailable' || rc.remoteReadsEnabled)) ||
      (sc && (sc.isRemote || (sc.capabilities && sc.capabilities.remote)))
    );

    const betaId = (typeof window !== 'undefined' && window.BetaIdentityProvider)
      ? window.BetaIdentityProvider.getCurrentIdentity()
      : null;
    const betaRole = (betaId && betaId.role) ? String(betaId.role).toUpperCase() : '';

    if (isRemoteMode) {
      // In REMOTE/MOCK mode: authorization depends ONLY on current BetaIdentityProvider persona
      // Remote Shared Core member data, state.role, circleRole, cached member role, or mock role NEVER elevate
      return betaRole === 'ADMIN' || betaRole === 'SUPERADMIN';
    }

    // Normal LOCAL prototype mode: preserve historical behavior
  ...
  function getActiveMemberInfo() {
    // If BetaIdentityProvider is active, harmonize with current canonical persona
    if (typeof BetaIdentityProvider !== 'undefined' && typeof BetaIdentityProvider.getCurrentIdentity === 'function') {
      const current = BetaIdentityProvider.getCurrentIdentity();
      if (current) {
        return {
          id: current.memberId || current.id || 'youth_demo_01',
          name: current.displayName || current.name || 'Alex Rivera',
          username: current.username || (current.id ? current.id.replace('youth_', '') : 'alex_r'),
          role: current.role || 'MEMBER',
          ageGroup: current.safeguards?.isMinor ? 'YOUTH' : 'ADULT',
          avatar: current.avatar || '🧑',
          bio: current.description || 'Young Pilgrim walking in faith and stewardship.'
        };
      }
    }
    // If development controller has cached remote member data, use it
    if (typeof window !== 'undefined' && window.sharedCoreAsyncController) {
      const cached = window.sharedCoreAsyncController.cachedData.member;
    ...
    const member = (sc && typeof sc.getCurrentMember === 'function')
      ? sc.getCurrentMember()
      : null;
    if (member && typeof member === 'object' && typeof member.then !== 'function' && member.id && !member.error) {
      return member;
    }
    return {
      id: 'youth_demo_01',
  ...
  function updateProfileMemberDetails() {
    ...
    // Check if canonical growth profile is available (Phase 0.23J)
    let growthProfile = null;
    if (typeof KoinoniaGrowth !== 'undefined' && typeof KoinoniaGrowth.getGrowthProfile === 'function') {
      const activeId = (typeof BetaIdentityProvider !== 'undefined') ? BetaIdentityProvider.getCurrentIdentity() : member;
      growthProfile = KoinoniaGrowth.getGrowthProfile(activeId);
    }

    const nameEl = document.getElementById('me-char-name');
    if (nameEl) nameEl.textContent = growthProfile ? growthProfile.identity.displayName : (member.name || 'Alex Rivera');

    const idEl = document.getElementById('me-char-identity-tag');
    if (idEl) idEl.textContent = `${(growthProfile ? growthProfile.memberId : member.id) || 'youth_demo_01'} • @${member.username || 'alex_r'}`;

    const avatarEl = document.getElementById('me-char-avatar');
    if (avatarEl) avatarEl.textContent = (growthProfile && growthProfile.identity.avatarEmoji) ? growthProfile.identity.avatarEmoji : (member.avatar || '🧑');
    ...
    // Level Title & LP
    const lvlTitleEl = document.getElementById('me-char-level-title');
    const curLevel = growthProfile ? growthProfile.progression.level : (state.charLevel || 1);
    const curTitle = growthProfile ? growthProfile.progression.levelTitle : 'New Explorer';
    if (lvlTitleEl) lvlTitleEl.textContent = `Level ${curLevel} • ${curTitle}`;

    const lpEl = document.getElementById('me-char-lp');
    const curLp = growthProfile ? growthProfile.progression.lifePoints : (state.lp || 0);
    if (lpEl) lpEl.textContent = `${curLp} LP`;

    // XP Progress
    const curXp = growthProfile ? growthProfile.progression.xp : (state.charXp || 0);
    const nextXp = growthProfile ? growthProfile.progression.nextLevelXp : (state.charXpMax || 10);
    const pct = growthProfile ? growthProfile.progression.progressPercent : Math.min(100, Math.round((curXp / Math.max(1, nextXp)) * 100));

    const xpLabel = document.getElementById('me-char-xp-label');
    if (xpLabel) xpLabel.textContent = `LEVEL ${curLevel} • ${curXp} / ${nextXp} XP (${pct}%)`;

    const xpNext = document.getElementById('me-char-xp-next');
    if (xpNext) {
      const remaining = growthProfile ? (growthProfile.progression.xpRemaining || 0) : Math.max(0, nextXp - curXp);
      xpNext.textContent = `Next Level: ${nextXp} XP (${remaining} needed)`;
    }

    const xpBar = document.getElementById('me-char-xp-bar');
    if (xpBar) xpBar.style.width = `${pct}%`;

    // Growth Journey Card (Phase 0.23J)
    const journeyLevelChip = document.getElementById('me-journey-level-chip');
    if (journeyLevelChip) journeyLevelChip.textContent = `Level ${curLevel}`;

    const journeyGoalText = document.getElementById('me-journey-goal-text');
    if (journeyGoalText) {
      const goals = {
        1: 'Next Growth Step: Complete your first daily quest in the Community Center to begin your journey.',
        2: 'Next Growth Step: Connect with a Campfire circle and practice daily stewardship duties.',
        3: 'Next Growth Step: Deepen community service and participate in fellowship gatherings.',
        4: 'Next Growth Step: Mentor younger pilgrims and contribute to community place stewardship.',
        5: 'Next Growth Step: Sustain leadership diligence, encourage peers, and oversee fellowship.'
      };
      journeyGoalText.innerHTML = `<strong>Next Growth Step:</strong> ${goals[curLevel] || goals[1]}`;
    }

    // Campfire & Ministry Info from Growth Profile
    const campfireEl = document.getElementById('me-char-campfire-info');
    if (campfireEl) {
      if (growthProfile && Array.isArray(growthProfile.memberships.campfires) && growthProfile.memberships.campfires.length > 0) {
        const cfText = growthProfile.memberships.campfires.map(c => `${c.name} (${c.role || 'Member'})`).join(', ');
        campfireEl.innerHTML = `🔥 <strong>Campfire:</strong> ${cfText}`;
      } else {
        const sc = getSharedCore();
    ...
    if (ministryEl) {
      if (growthProfile && Array.isArray(growthProfile.memberships.ministries) && growthProfile.memberships.ministries.length > 0) {
        const mText = growthProfile.memberships.ministries.map(m => `${m.name} (${m.role || 'Member'})`).join(', ');
        ministryEl.innerHTML = `⛪ <strong>Ministry:</strong> ${mText}`;
      } else {
        const sc = getSharedCore();
  ...
  function markEventAttended(eventId, reflection = '') {
    ...
    // Event rewards: Routed strictly through KoinoniaRewardAuthority
    const activeMemberId = (typeof BetaIdentityProvider !== 'undefined' && typeof BetaIdentityProvider.getCurrentIdentity === 'function')
      ? (BetaIdentityProvider.getCurrentIdentity()?.memberId || 'youth_demo_01')
      : 'youth_demo_01';

    let eventRewardResult = null;
    if (typeof KoinoniaRewardAuthority !== 'undefined' && typeof KoinoniaRewardAuthority.recordEventAttendance === 'function') {
      eventRewardResult = KoinoniaRewardAuthority.recordEventAttendance(activeMemberId, eventId, {
        reflection: reflection || '',
        title: title
      });
    }

    if (eventRewardResult && eventRewardResult.success && !eventRewardResult.alreadyCompleted) {
      // Absolute hydration from KoinoniaGrowth read model (zero UI delta arithmetic)
      const profile = (typeof KoinoniaGrowth !== 'undefined' && typeof KoinoniaGrowth.getGrowthProfile === 'function')
        ? KoinoniaGrowth.getGrowthProfile(activeMemberId)
        : null;
  ...
  // ============================================================
  // 18. PROTOTYPE STATE RESET (DEVELOPER RESET - PATH D QUARANTINED)
  // ============================================================
  function resetPrototypeState() {
    ...
    // Reset member state in Reward Authority
    if (typeof KoinoniaRewardAuthority !== 'undefined' && typeof KoinoniaRewardAuthority.resetMemberState === 'function') {
      KoinoniaRewardAuthority.resetMemberState(activeMemberId);
    }

    // Hydrate baseline progression from KoinoniaGrowth
    const baseGrowth = (typeof KoinoniaGrowth !== 'undefined' && typeof KoinoniaGrowth.getGrowthProfile === 'function')
      ? KoinoniaGrowth.getGrowthProfile(activeMemberId)
      : null;

    state.activePlaceId = 'home';
    state.spawnId = 'default';
    state.lp = (baseGrowth && baseGrowth.progression && typeof baseGrowth.progression.lifePoints === 'number')
      ? baseGrowth.progression.lifePoints
      : 120;
    state.charLevel = (baseGrowth && baseGrowth.progression && typeof baseGrowth.progression.level === 'number')
      ? baseGrowth.progression.level
      : 1;
    state.charXp = (baseGrowth && baseGrowth.progression && typeof baseGrowth.progression.xp === 'number')
      ? baseGrowth.progression.xp
      : 0;
    state.charXpMax = (baseGrowth && baseGrowth.progression && typeof baseGrowth.progression.nextLevelXp === 'number')
      ? baseGrowth.progression.nextLevelXp
      : 10;
    state.highestLevelReached = state.charLevel;
  ...
  // ============================================================
  // 19. KOINONIA STUDIO DASHBOARD & GOVERNANCE ENGINE (PHASE 0.23F)
  // ============================================================
  function getStudioUserIdentity() {
    if (typeof KoinoniaIdentity !== 'undefined') {
      const current = KoinoniaIdentity.getCurrentIdentity();
      if (current) {
        return {
          memberId: current.memberId,
          id: current.memberId || current.id,
          name: current.displayName || current.name,
          displayName: current.displayName || current.name,
          shortName: current.shortName || current.displayName || current.name,
          role: (current.role || 'MEMBER').toUpperCase(),
          avatar: (current.profile && current.profile.avatarEmoji) || current.avatar || '🧑',
          badge: (current.profile && current.profile.badge) || current.badge || current.role,
          title: (current.profile && current.profile.title) || current.title,
          capabilities: current.capabilities,
          canAccessStudio: KoinoniaIdentity.canAccessStudio(current),
          canAuthorContent: KoinoniaIdentity.canAuthorContent(current),
          canReviewContent: KoinoniaIdentity.canReviewContent(current),
          canPublishContent: KoinoniaIdentity.canPublishContent(current),
          canManageCampfire: KoinoniaIdentity.canManageCampfire(current),
    ...
    if (typeof BetaIdentityProvider !== 'undefined') {
      ...
      if (current) {
        return {
          memberId: current.memberId || current.id,
          id: current.id || current.memberId,
          name: current.name || current.displayName,
          displayName: current.displayName || current.name,
          shortName: current.shortName || current.name,
          role: (current.role || 'MEMBER').toUpperCase(),
          avatar: current.avatar || '🧑',
          badge: current.badge || current.role,
          title: current.title,
          capabilities: current.capabilities,
          canAccessStudio: BetaIdentityProvider.canAccessStudio(current)
        };
      }
    }
    const role = (state.role || 'MEMBER').toUpperCase();
    return {
      memberId: 'unknown_user',
      id: 'unknown_user',
      name: state.charName || 'Anonymous',
      displayName: state.charName || 'Anonymous',
      shortName: state.charName || 'Anonymous',
      role,
      avatar: '🧑',
      badge: state.role || 'MEMBER',
      canAccessStudio: ['ADMIN', 'SUPERADMIN'].includes(role)
    };
  }

  function openAdminStudio() {
    const hasAccess = (typeof KoinoniaIdentity !== 'undefined')
      ? KoinoniaIdentity.canAccessStudio(KoinoniaIdentity.getCurrentIdentity())
      : (typeof BetaIdentityProvider !== 'undefined' ? BetaIdentityProvider.canAccessStudio() : false);
    if (!hasAccess) {
  ...
  function switchStudioView(viewName) {
    const identity = getStudioUserIdentity();
    if (viewName === 'review' && identity.role !== 'SUPERADMIN') {
      showToast('🔒 Review Queue is reserved for Superadmin oversight.');
      viewName = 'dashboard';
    }

    if (viewName === 'preview') {
      state.previousStudioView = state.studioView || 'dashboard';
    }
    state.studioView = viewName;

    if (typeof document === 'undefined') return;

    const views = ['dashboard', 'create-picker', 'wizard', 'drafts', 'submitted', 'review', 'published', 'media', 'audit', 'preview'];
    views.forEach(v => {
      const el = document.getElementById(`studio-view-${v}`);
      if (el) el.style.display = (v === viewName) ? 'block' : 'none';
    });

    const subnavMap = {
      dashboard: 'nav-studio-home',
      'create-picker': 'nav-studio-create',
      drafts: 'nav-studio-drafts',
      submitted: 'nav-studio-submitted',
      review: 'nav-studio-review',
      published: 'nav-studio-published',
      media: 'nav-studio-media',
      audit: 'nav-studio-audit'
    };
    Object.values(subnavMap).forEach(id => {
      const btn = document.getElementById(id);
      if (btn) btn.classList.remove('active');
    });
    const activeNavId = subnavMap[viewName];
    if (activeNavId) {
    ...
    updateStudioIdentityBanner(identity);

    if (viewName === 'dashboard') renderStudioDashboard();
    else if (viewName === 'wizard') updateWizardUI();
    else if (viewName === 'drafts') renderStudioDrafts();
    else if (viewName === 'submitted') renderStudioSubmitted();
    else if (viewName === 'review') renderStudioReviewQueue();
    else if (viewName === 'published') renderStudioPublished();
    else if (viewName === 'media') renderStudioMedia();
    else if (viewName === 'audit') renderStudioAudit();
  }

  function updateStudioIdentityBanner(identity) {
    if (typeof document === 'undefined') return;
    if (!identity) identity = getStudioUserIdentity();
    const nameEl = document.getElementById('studio-user-name');
    const roleEl = document.getElementById('studio-user-role');
    const avatarEl = document.getElementById('studio-user-avatar');
    ...
    if (avatarEl) {
      avatarEl.textContent = identity.avatar || (identity.role === 'SUPERADMIN' ? '👨‍🦳' : '👩‍💼');
      avatarEl.innerText = avatarEl.textContent;
    }

    if (roleEl) {
      if (identity.role === 'ADMIN') {
        roleEl.textContent = 'ADMIN • CONTENT AUTHOR';
        roleEl.innerText = 'ADMIN • CONTENT AUTHOR';
        roleEl.style.color = '#B45309';
      } else if (identity.role === 'SUPERADMIN') {
        roleEl.textContent = 'SUPERADMIN • APPROVER / PUBLISHER';
        roleEl.innerText = 'SUPERADMIN • APPROVER / PUBLISHER';
        roleEl.style.color = 'var(--brand-burgundy, #6A0E04)';
      } else {
        roleEl.textContent = 'MEMBER';
        roleEl.innerText = 'MEMBER';
        roleEl.style.color = '#6B7280';
      }
    }

    const superOnlyEls = document.querySelectorAll('.superadmin-only');
    superOnlyEls.forEach(el => {
      el.style.display = (identity.role === 'SUPERADMIN') ? '' : 'none';
    });
  }

  function updateStudioCounts() {
    if (typeof document === 'undefined') return;
    if (typeof KoinoniaStudio === 'undefined' || !KoinoniaStudio.store) return;

    const allDrafts = KoinoniaStudio.store.listDrafts() || [];
    const draftsCount = allDrafts.filter(d => d.status === 'DRAFT').length;
    const submittedCount = allDrafts.filter(d => d.status === 'READY_FOR_REVIEW').length;
    const readyCount = allDrafts.filter(d => d.status === 'APPROVED').length;
    const publishedCount = (KoinoniaStudio.store.listPublished() || []).length;
    ...
    setText('stat-count-drafts', draftsCount);
    setText('stat-count-submitted', submittedCount);
    setText('stat-count-review', submittedCount);
    setText('stat-count-ready', readyCount);
    setText('stat-count-published', publishedCount);

    setText('nav-count-drafts', draftsCount);
    setText('nav-count-submitted', submittedCount);
    setText('nav-count-review', submittedCount + readyCount);
    setText('nav-count-published', publishedCount);
  }
  ...
  function launchWizardForTemplate(templateKey) {
    state.activeStudioTemplate = templateKey || 'QUEST';
    state.activeDraftId = null;
    state.wizardStep = 1;

    if (typeof document !== 'undefined') {
      const badge = document.getElementById('wizard-active-template-badge');
      const tmplLabels = {
        QUEST: '📜 QUEST WIZARD',
        EVENT: '📅 EVENT WIZARD',
        CAMPAIGN: '🧭 CAMPAIGN WIZARD',
        CAMPFIRE_ACTIVITY: '🔥 CAMPFIRE WIZARD',
        MINISTRY_MISSION: '🌾 MISSION WIZARD'
      };
      if (badge) badge.textContent = tmplLabels[templateKey] || `${templateKey} WIZARD`;

      const nameInput = document.getElementById('wizard-place-name');
      if (nameInput) {
        if (templateKey === 'QUEST') nameInput.value = 'Sunset Intercession';
        else if (templateKey === 'EVENT') nameInput.value = 'Youth Worship Gathering';
        else if (templateKey === 'CAMPAIGN') nameInput.value = 'Fruit of the Spirit Journey';
        else if (templateKey === 'CAMPFIRE_ACTIVITY') nameInput.value = 'Gratitude Campfire Circle';
        else if (templateKey === 'MINISTRY_MISSION') nameInput.value = 'Community Care Outreach';
        else nameInput.value = 'Prayer Garden';
      }

      const feedbackEl = document.getElementById('wizard-save-feedback');
      if (feedbackEl) {
        feedbackEl.style.display = 'none';
        feedbackEl.textContent = '';
        feedbackEl.style.opacity = '1';
      }
      const saveBtn = document.getElementById('btn-wizard-save-draft');
      if (saveBtn) {
        saveBtn.innerHTML = saveBtn.getAttribute('data-orig-text') || '💾 Save Draft';
    ...
    switchStudioView('wizard');
  }

  function returnFromWizardToHome() {
    switchStudioView('dashboard');
  }
  ...
  function updateWizardUI() {
    ...
    const titleLabel = document.getElementById('wizard-step-title-label');
    const stepTitles = {
      1: 'Place & Identity',
      2: 'Template & Category',
      3: 'Zone & Context',
    ...
    if (titleLabel) titleLabel.textContent = stepTitles[state.wizardStep] || 'Step ' + state.wizardStep;
    ...
    if (prevBtn) {
      prevBtn.style.display = (state.wizardStep > 1) ? 'block' : 'none';
    }

    const nextBtn = document.getElementById('btn-wizard-next');
    if (nextBtn) {
      nextBtn.textContent = (state.wizardStep === 7) ? 'Review Ready' : 'Next Step';
    }

    const identity = getStudioUserIdentity();
    const approveBtn = document.getElementById('btn-wizard-approve');
    const publishBtn = document.getElementById('btn-wizard-publish');
    if (approveBtn) {
      approveBtn.style.display = (identity.role === 'SUPERADMIN' && state.wizardStep === 7) ? 'block' : 'none';
    }
    if (publishBtn) {
      publishBtn.style.display = (identity.role === 'SUPERADMIN' && state.wizardStep === 7) ? 'block' : 'none';
    }
  }
  ...
  function getWizardFormData() {
    ...
    const title = getVal('wizard-place-name', 'Custom Place');
    const template = getVal('wizard-place-template', 'courtyard');
    const zone = getVal('wizard-place-zone', 'East Reflection Bower');
    const npcName = getVal('wizard-npc-name', 'Sister Grace');
    const questTitle = getVal('wizard-quest-title', 'Sunset Intercession');
    const verifyType = getVal('wizard-quest-verify', 'TRUST');

    const verificationMethodMap = {
      'TRUST': 'self_reflection',
      'FAMILY': 'family_affirmation',
      'LEADER': 'leader_checkin'
    };
    ...
    return {
      ...
      questTitle,
      verifyType,
      shortDesc: `Custom experience authored in Koinonia Studio for ${zone}.`,
      longDesc: `A safe church fellowship experience featuring ${npcName} and calling '${questTitle}'.`,
      category: 'Faith & Stewardship',
      placeId: 'fog_center',
      audience: 'all',
      difficulty: 'medium',
      estimatedMinutes: 30,
      realWorldAction: `Fellowship and stewardship action at ${zone} guided by ${npcName}.`,
      reflectionPrompt: `How did your participation in '${questTitle}' deepen your walk in faith and community?`,
      verificationMethod: verificationMethodMap[verifyType] || 'self_reflection',
      lifePoints: 10,
      characterXp: 20,
      skillXp: 15,
      completionMessage: `Well done! Your fellowship experience at ${title} has been completed.`,
      startDate: new Date().toISOString().split('T')[0],
      presentationTarget: 'KOINONIA'
    };
  }

  function handleSaveStudioDraft() {
    const identity = getStudioUserIdentity();
    const saveBtn = typeof document !== 'undefined' ? document.getElementById('btn-wizard-save-draft') : null;
    const feedbackEl = typeof document !== 'undefined' ? document.getElementById('wizard-save-feedback') : null;

    if (!identity.canAccessStudio) {
      if (feedbackEl) {
        ...
        feedbackEl.textContent = '⚠️ Studio access requires Admin or Superadmin role.';
      }
      showToast('⚠️ Studio access requires Admin or Superadmin role.');
      return;
    }

    if (typeof KoinoniaStudio === 'undefined' || !KoinoniaStudio.store) {
      if (feedbackEl) {
        feedbackEl.style.display = 'block';
        feedbackEl.style.opacity = '1';
        feedbackEl.style.background = '#FEE2E2';
        feedbackEl.style.color = '#991B1B';
        feedbackEl.style.border = '1px solid #FCA5A5';
        feedbackEl.textContent = '⚠️ Studio engine not initialized.';
      }
      showToast('⚠️ Studio engine not initialized.');
      return;
    }
    ...
    if (!formData.title) {
      if (feedbackEl) {
        ...
        feedbackEl.style.border = '1px solid #FCD34D';
        feedbackEl.textContent = '⚠️ Please enter a title or place name before saving.';
      }
      showToast('⚠️ Please enter a title or place name.');
      return;
    }
    ...
    try {
      if (state.activeDraftId && KoinoniaStudio.store.getDraft(state.activeDraftId)) {
        KoinoniaStudio.store.updateDraft(state.activeDraftId, formData, actor);
      } else {
        const tmpl = state.activeStudioTemplate || 'QUEST';
        const draft = KoinoniaStudio.store.createDraft(tmpl, formData, actor);
        state.activeDraftId = draft.id;
      }
      updateStudioCounts();

      // 1. Temporary button state change: "✓ Draft Saved"
      if (saveBtn) {
        if (!saveBtn.getAttribute('data-orig-text')) {
          saveBtn.setAttribute('data-orig-text', saveBtn.innerHTML || '💾 Save Draft');
        }
        saveBtn.innerHTML = '✓ Draft Saved';
        saveBtn.style.background = '#166534';
        saveBtn.style.color = '#FFFFFF';
        saveBtn.style.borderColor = '#22C55E';

        if (state._saveBtnTimeout) clearTimeout(state._saveBtnTimeout);
        state._saveBtnTimeout = setTimeout(() => {
          if (saveBtn) {
            saveBtn.innerHTML = saveBtn.getAttribute('data-orig-text') || '💾 Save Draft';
            saveBtn.style.background = '';
            saveBtn.style.color = '';
      ...
      // 2. Visible inline feedback banner: "✓ Draft saved"
      if (feedbackEl) {
        feedbackEl.style.display = 'block';
        feedbackEl.style.opacity = '1';
        ...
        feedbackEl.innerHTML = '✓ Draft saved &bull; <span style="font-weight: 500; font-size: 0.75rem;">Continuing in wizard</span>';
        if (state._saveFeedbackTimeout) clearTimeout(state._saveFeedbackTimeout);
        state._saveFeedbackTimeout = setTimeout(() => {
          if (feedbackEl) {
      ...
      if (saveBtn) {
        saveBtn.innerHTML = '⚠️ Save Failed';
      ...
      if (feedbackEl) {
        ...
        feedbackEl.style.color = '#991B1B';
        feedbackEl.style.border = '1px solid #FCA5A5';
        feedbackEl.textContent = '⚠️ Failed to save draft: ' + err.message;
      }
      showToast('⚠️ Failed to save draft: ' + err.message);
    }
  }

  function handleSubmitStudioDraftForReview() {
    if (state.isStudioSubmitting) return;
    const identity = getStudioUserIdentity();
    if (typeof KoinoniaStudio === 'undefined' || !KoinoniaStudio.store) return;

    handleSaveStudioDraft();
    ...
    const submitBtn = document.getElementById('btn-wizard-submit-review');
    if (submitBtn) submitBtn.disabled = true;
    state.isStudioSubmitting = true;

    try {
      KoinoniaStudio.store.requestReview(state.activeDraftId, actor);
      updateStudioCounts();
      showToast('📨 Content submitted for Superadmin review!');
      switchStudioView('submitted');
    } catch (err) {
      showToast('⚠️ Submit failed: ' + err.message);
    } finally {
      state.isStudioSubmitting = false;
      if (submitBtn) submitBtn.disabled = false;
    }
  }

  function handleApproveStudioDraft(draftId) {
    const identity = getStudioUserIdentity();
    if (identity.role !== 'SUPERADMIN') {
      showToast('🔒 Only Superadmin leaders can approve content.');
      return;
    }

    const targetId = draftId || state.activeDraftId;
    if (!targetId || typeof KoinoniaStudio === 'undefined' || !KoinoniaStudio.store) return;
    ...
    try {
      KoinoniaStudio.store.approveDraft(targetId, actor);
      updateStudioCounts();
      showToast('✅ Content approved! Ready to publish.');
      if (state.studioView === 'review') renderStudioReviewQueue();
      else if (state.studioView === 'wizard') updateWizardUI();
    } catch (err) {
      showToast('⚠️ Approval failed: ' + err.message);
    }
  }

  function openReturnForChangesModal(draftId) {
    const identity = getStudioUserIdentity();
    if (identity.role !== 'SUPERADMIN') {
      showToast('🔒 Only Superadmin leaders can return content.');
      return;
    }
    ...
    const draft = KoinoniaStudio.store.getDraft(targetId);
    if (!draft) {
      showToast('⚠️ Draft not found.');
      return;
    }

    if (draft.status !== 'READY_FOR_REVIEW' && draft.status !== 'APPROVED') {
      showToast(`⚠️ Cannot return content in '${draft.status}' state.`);
      return;
    }

    state.pendingReturnDraftId = targetId;

    if (typeof document !== 'undefined') {
      const modal = document.getElementById('modal-studio-return-changes');
      const input = document.getElementById('return-changes-note-input');
      const errEl = document.getElementById('return-changes-error');
      const countEl = document.getElementById('return-changes-char-count');
      const confirmBtn = document.getElementById('btn-confirm-return-changes');
      ...
      if (input) {
        ...
        input.oninput = function() {
          ...
          if (errEl && input.value.trim().length >= 3) {
      ...
      if (confirmBtn) {
        ...
        confirmBtn.textContent = 'Return for Changes';
      }
  ...
  function closeReturnForChangesModal() {
    state.pendingReturnDraftId = null;
    state.isReturningChanges = false;
    if (typeof document !== 'undefined') {
      const modal = document.getElementById('modal-studio-return-changes');
      const input = document.getElementById('return-changes-note-input');
      const errEl = document.getElementById('return-changes-error');
  ...
  function confirmReturnForChanges(overrideNote) {
    if (state.isReturningChanges) return;
    ...
    const targetId = state.pendingReturnDraftId;
    if (!targetId || typeof KoinoniaStudio === 'undefined' || !KoinoniaStudio.store) {
      showToast('⚠️ No draft selected for return.');
      return;
    }

    let rawNote = '';
    if (typeof overrideNote === 'string' && overrideNote.trim().length > 0) {
      rawNote = overrideNote;
    } else if (typeof document !== 'undefined') {
      const input = document.getElementById('return-changes-note-input');
      if (input) rawNote = input.value;
    }

    const trimmed = (rawNote || '').trim();
    const sanitizedNote = trimmed.replace(/<[^>]*>/g, '').trim();

    const errEl = typeof document !== 'undefined' ? document.getElementById('return-changes-error') : null;
    const confirmBtn = typeof document !== 'undefined' ? document.getElementById('btn-confirm-return-changes') : null;

    if (sanitizedNote.length < 3) {
      if (errEl) {
        errEl.textContent = 'Please enter a note of at least 3 characters explaining the requested changes.';
        errEl.style.display = 'block';
      }
      showToast('⚠️ Note must be at least 3 characters.');
      return;
    }

    if (sanitizedNote.length > 500) {
      if (errEl) {
        errEl.textContent = 'Note cannot exceed 500 characters.';
        errEl.style.display = 'block';
      }
      showToast('⚠️ Note cannot exceed 500 characters.');
      return;
    }
    ...
    try {
      KoinoniaStudio.store.returnForChanges(targetId, actor, sanitizedNote);
      updateStudioCounts();
      closeReturnForChangesModal();
      showToast('✓ Returned for changes');

      if (state.studioView === 'review') renderStudioReviewQueue();
      else if (state.studioView === 'submitted') renderStudioSubmitted();
      else if (state.studioView === 'wizard') updateWizardUI();
    } catch (err) {
      if (errEl) {
        errEl.textContent = '⚠️ Return failed: ' + err.message;
        errEl.style.display = 'block';
      }
  ...
  function handleReturnStudioDraftForChanges(draftId, note) {
    if (note && typeof note === 'string' && note.trim().length >= 3) {
      state.pendingReturnDraftId = draftId;
      confirmReturnForChanges(note);
    } else {
      openReturnForChangesModal(draftId);
    }
  }

  function handlePublishStudioDraft(draftId) {
    const identity = getStudioUserIdentity();
    if (identity.role !== 'SUPERADMIN') {
      showToast('🔒 Only Superadmin leaders can publish content.');
      return;
    }
    ...
    try {
      ...
      if (draft.status !== 'APPROVED') {
        showToast('⚠️ Content must be APPROVED before publishing.');
        return;
      }
      const res = KoinoniaStudio.store.publishDraft(targetId, actor);

      if (typeof PLACES !== 'undefined' && draft && draft.data && draft.data.placeName) {
        const placeId = 'custom_' + Date.now();
        PLACES[placeId] = {
          id: placeId,
          name: draft.data.placeName,
          category: draft.data.category || 'Community Fellowship',
          icon: '🏛️',
          tagline: draft.data.shortDesc || 'Custom Community Space',
          unlocked: true,
          spawn: { x: 12, y: 14, dir: 'up' },
          interactables: []
        };
        if (state.unlockedPlaces && !state.unlockedPlaces.includes(placeId)) {
          state.unlockedPlaces.push(placeId);
        }
        // Persist to custom places store for refresh resilience
        try {
          const rawPlaces = localStorage.getItem('koinonia_phase21_custom_places');
          const customPlaces = rawPlaces ? JSON.parse(rawPlaces) : [];
          if (Array.isArray(customPlaces)) {
            customPlaces.push(PLACES[placeId]);
            localStorage.setItem('koinonia_phase21_custom_places', JSON.stringify(customPlaces));
          }
        } catch (_) {}
      }

      updateStudioCounts();
      showToast('🎉 Content published successfully to world!');
      switchStudioView('published');
    } catch (err) {
      showToast('⚠️ Publish failed: ' + err.message);
    }
  }

  function handleDeleteStudioDraft(draftId) {
    const identity = getStudioUserIdentity();
    if (typeof KoinoniaStudio === 'undefined' || !KoinoniaStudio.store) return;
    ...
    try {
      KoinoniaStudio.store.deleteDraft(draftId, actor, true);
  ...
  function handlePreviewStudioDraft(draftId) {
    if (typeof KoinoniaStudio === 'undefined') return;

    let templateType = state.activeStudioTemplate || 'QUEST';
    let data = null;

    if (draftId && KoinoniaStudio.store) {
      const draft = KoinoniaStudio.store.getDraft(draftId);
      if (draft) {
    ...
    if (!data) {
      data = getWizardFormData();
    }

    try {
      const previewModel = KoinoniaStudio.generatePreviewModel(templateType, data);
      const container = document.getElementById('studio-preview-content');
      if (container) {
        container.innerHTML = `
          <div style="background: linear-gradient(135deg, #FFF9F3 0%, #FFF2E5 100%); border: 1.5px solid var(--border-gold, #D97706); border-radius: 12px; padding: 14px; margin-bottom: 14px; text-align: center;">
            <div style="font-size: 0.72rem; font-weight: 800; color: #92400E; text-transform: uppercase;">${previewModel.banner.text}</div>
            <div style="font-family: var(--font-devotional, serif); font-size: 1.25rem; font-weight: 800; color: var(--brand-burgundy, #6A0E04); margin: 4px 0;">${previewModel.title}</div>
            <div style="font-size: 0.74rem; color: #4B5563; line-height: 1.4;">${previewModel.banner.warning}</div>
          </div>

          <div style="background: #FFFFFF; border: 1px solid var(--border-light); border-radius: 12px; padding: 14px; margin-bottom: 14px;">
            <div style="font-size: 0.72rem; font-weight: 700; color: #6B7280; text-transform: uppercase;">Preview Data</div>
            <div style="font-size: 0.88rem; font-weight: 700; color: var(--brand-charcoal); margin-top: 4px;">Template: ${previewModel.templateLabel}</div>
            <div style="font-size: 0.8rem; color: #4B5563; margin-top: 4px;">Place / Zone: ${data.placeName || 'Prayer Garden'} (${data.zone || 'Reflection Bower'})</div>
            <div style="font-size: 0.8rem; color: #4B5563; margin-top: 4px;">NPC Elder: ${data.npcName || 'Sister Grace'}</div>
            <div style="font-size: 0.8rem; color: #4B5563; margin-top: 4px;">Calling / Prompt: ${data.questTitle || 'Sunset Intercession'}</div>
            <div style="font-size: 0.8rem; color: #4B5563; margin-top: 4px;">Verification: ${data.verifyType || 'TRUST'}</div>
          </div>

          <div style="background: #F0FDF4; border: 1px solid #BBF7D0; border-radius: 10px; padding: 10px 12px; font-size: 0.74rem; color: #166534;">
            🛡️ <strong>Zero-Mutation Invariant:</strong> LP Awarded: 0 | XP Awarded: 0 | Attendance Recorded: No
          </div>
        `;
      }
  ...
  function closeStudioPreview() {
    switchStudioView(state.previousStudioView || 'dashboard');
  }

  function renderStudioDrafts() {
    if (typeof document === 'undefined') return;
    const container = document.getElementById('studio-drafts-list');
    if (!container) return;

    if (typeof KoinoniaStudio === 'undefined' || !KoinoniaStudio.store) {
      container.innerHTML = '<div class="studio-empty-state"><div class="studio-empty-text">Studio engine not available.</div></div>';
      return;
    }

    const drafts = (KoinoniaStudio.store.listDrafts() || []).filter(d => d.status === 'DRAFT');
    if (drafts.length === 0) {
      container.innerHTML = `
        <div class="studio-empty-state">
          <div class="studio-empty-icon">📝</div>
          <div class="studio-empty-text">No drafts yet.</div>
          <div class="studio-empty-sub">Press + Create New to start authoring content.</div>
        </div>
      `;
      return;
    }
    ...
    container.innerHTML = drafts.map(d => `
      <div class="studio-card-item">
        <div class="studio-card-header">
          <div>
            <div style="font-weight: 800; font-size: 0.9rem; color: var(--brand-burgundy);">${d.title}</div>
            <div style="font-size: 0.70rem; color: #6B7280;">Template: ${d.templateType} • Updated: ${new Date(d.updatedAt).toLocaleTimeString()}</div>
          </div>
          <span class="studio-card-badge ${d.reviewNotes ? 'studio-badge-review' : 'studio-badge-draft'}">
            ${d.reviewNotes ? 'CHANGES REQUESTED' : 'DRAFT'}
          </span>
        </div>
        ${d.reviewNotes ? `
          <div style="background: #FFFBEB; border-left: 3px solid #F59E0B; padding: 6px 10px; margin: 6px 0; font-size: 0.72rem; color: #92400E; border-radius: 4px;">
            <strong>Leadership Note:</strong> ${d.reviewNotes}
          </div>
        ` : ''}
        <div class="studio-card-actions">
          <button class="secondary-btn btn-sm" onclick="window.KOINONIA_GAME.resumeDraftInWizard('${d.id}')">✏️ Edit in Wizard</button>
          <button class="secondary-btn btn-sm" onclick="window.KOINONIA_GAME.handlePreviewStudioDraft('${d.id}')">👁️ Preview</button>
          <button class="primary-btn btn-sm" onclick="window.KOINONIA_GAME.submitDraftDirectly('${d.id}')">📨 Submit Review</button>
          <button class="secondary-btn btn-sm" style="color: #DC2626;" onclick="window.KOINONIA_GAME.handleDeleteStudioDraft('${d.id}')">🗑️ Delete</button>
        </div>
      </div>
    `).join('');
  }

  function resumeDraftInWizard(draftId) {
    if (typeof KoinoniaStudio === 'undefined' || !KoinoniaStudio.store) return;
    const draft = KoinoniaStudio.store.getDraft(draftId);
    if (!draft) return;

    state.activeStudioTemplate = draft.templateType;
    state.activeDraftId = draft.id;
    state.wizardStep = 1;

    const nameInput = document.getElementById('wizard-place-name');
    if (nameInput && draft.data && (draft.data.placeName || draft.title)) {
      nameInput.value = draft.data.placeName || draft.title;
    }

    const templateSelect = document.getElementById('wizard-place-template');
    if (templateSelect && draft.data && (draft.data.template || draft.data.placeTemplate)) {
      templateSelect.value = draft.data.template || draft.data.placeTemplate;
    }

    const zoneInput = document.getElementById('wizard-place-zone');
    if (zoneInput && draft.data && draft.data.zone) zoneInput.value = draft.data.zone;

    const npcInput = document.getElementById('wizard-npc-name');
    if (npcInput && draft.data && draft.data.npcName) npcInput.value = draft.data.npcName;

    const questInput = document.getElementById('wizard-quest-title');
    if (questInput && draft.data && draft.data.questTitle) questInput.value = draft.data.questTitle;

    const verifySelect = document.getElementById('wizard-quest-verify');
    if (verifySelect && draft.data && draft.data.verifyType) {
      verifySelect.value = draft.data.verifyType;
    }
  ...
  function submitDraftDirectly(draftId) {
    ...
    state.isStudioSubmitting = true;
    try {
      KoinoniaStudio.store.requestReview(draftId, actor);
      updateStudioCounts();
      showToast('📨 Content submitted for review!');
      switchStudioView('submitted');
    } catch (err) {
      showToast('⚠️ Submit failed: ' + err.message);
    } finally {
  ...
  function renderStudioSubmitted() {
    if (typeof document === 'undefined') return;
    const container = document.getElementById('studio-submitted-list');
    if (!container) return;
    ...
    const identity = getStudioUserIdentity();
    const isSuper = identity.role === 'SUPERADMIN';
    const submitted = (KoinoniaStudio.store.listDrafts() || []).filter(d => d.status === 'READY_FOR_REVIEW');

    if (submitted.length === 0) {
      container.innerHTML = `
        <div class="studio-empty-state">
          <div class="studio-empty-icon">📨</div>
          <div class="studio-empty-text">No content awaiting review.</div>
          <div class="studio-empty-sub">All submissions have been reviewed or none are pending.</div>
        </div>
      `;
      return;
    }

    container.innerHTML = submitted.map(d => `
      <div class="studio-card-item">
        <div class="studio-card-header">
          <div>
            <div style="font-weight: 800; font-size: 0.9rem; color: var(--brand-burgundy);">${d.title}</div>
            <div style="font-size: 0.70rem; color: #6B7280;">Author: ${d.createdBy ? d.createdBy.name : 'Leader'} • Submitted: ${new Date(d.updatedAt).toLocaleTimeString()}</div>
          </div>
          <span class="studio-card-badge studio-badge-review">SUBMITTED FOR REVIEW</span>
        </div>
        <p style="font-size: 0.74rem; color: #4B5563; margin: 4px 0 8px 0;">
          ${isSuper ? 'Pending your governance review.' : '⏳ Awaiting Superadmin review by Father Alex.'}
        </p>
        <div class="studio-card-actions">
          <button class="secondary-btn btn-sm" onclick="window.KOINONIA_GAME.handlePreviewStudioDraft('${d.id}')">👁️ Preview</button>
          ${isSuper ? `
            <button class="primary-btn btn-sm" style="background: #166534;" onclick="window.KOINONIA_GAME.handleApproveStudioDraft('${d.id}')">✅ Approve</button>
            <button class="secondary-btn btn-sm" onclick="window.KOINONIA_GAME.openReturnForChangesModal('${d.id}')">↩️ Return for Changes</button>
          ` : ''}
        </div>
      </div>
    `).join('');
  ...
  function renderStudioReviewQueue() {
    if (typeof document === 'undefined') return;
    const container = document.getElementById('studio-review-list');
    if (!container) return;
    ...
    const identity = getStudioUserIdentity();
    if (identity.role !== 'SUPERADMIN') {
      container.innerHTML = '<div class="studio-empty-state"><div class="studio-empty-text">Access restricted to Superadmin.</div></div>';
      return;
    }

    const queue = (KoinoniaStudio.store.listDrafts() || []).filter(d => d.status === 'READY_FOR_REVIEW' || d.status === 'APPROVED');
    if (queue.length === 0) {
      container.innerHTML = `
        <div class="studio-empty-state">
          <div class="studio-empty-icon">⚖️</div>
          <div class="studio-empty-text">Review queue is clear.</div>
          <div class="studio-empty-sub">No content awaiting review or ready to publish.</div>
        </div>
      `;
      return;
    }
    ...
    container.innerHTML = queue.map(d => `
      <div class="studio-card-item">
        <div class="studio-card-header">
          <div>
            <div style="font-weight: 800; font-size: 0.9rem; color: var(--brand-burgundy);">${d.title}</div>
            <div style="font-size: 0.70rem; color: #6B7280;">Author: ${d.createdBy ? d.createdBy.name : 'Leader'} • Template: ${d.templateType}</div>
          </div>
          <span class="studio-card-badge ${d.status === 'APPROVED' ? 'studio-badge-approved' : 'studio-badge-review'}">
            ${d.status === 'APPROVED' ? 'APPROVED • READY TO PUBLISH' : 'PENDING REVIEW'}
          </span>
        </div>
        <div class="studio-card-actions">
          <button class="secondary-btn btn-sm" onclick="window.KOINONIA_GAME.handlePreviewStudioDraft('${d.id}')">👁️ Preview</button>
          ${d.status === 'READY_FOR_REVIEW' ? `
            ...
            <button class="secondary-btn btn-sm" onclick="window.KOINONIA_GAME.openReturnForChangesModal('${d.id}')">↩️ Return for Changes</button>
          ` : `
            <button class="primary-btn btn-sm" style="background: var(--brand-burgundy);" onclick="window.KOINONIA_GAME.handlePublishStudioDraft('${d.id}')">👑 Publish Now</button>
          `}
        </div>
      </div>
    `).join('');
  ...
  function renderStudioPublished() {
    if (typeof document === 'undefined') return;
    const container = document.getElementById('studio-published-list');
    if (!container) return;

    if (typeof KoinoniaStudio === 'undefined' || !KoinoniaStudio.store) return;

    const published = KoinoniaStudio.store.listPublished() || [];
    if (published.length === 0) {
      container.innerHTML = `
        <div class="studio-empty-state">
          <div class="studio-empty-icon">✨</div>
          <div class="studio-empty-text">Nothing ready to publish.</div>
          <div class="studio-empty-sub">Approved content published by Father Alex will appear here.</div>
        </div>
      `;
      return;
    }
    ...
    container.innerHTML = published.map(p => `
      <div class="studio-card-item">
        <div class="studio-card-header">
          <div>
            <div style="font-weight: 800; font-size: 0.9rem; color: var(--brand-burgundy);">${p.title}</div>
            <div style="font-size: 0.70rem; color: #6B7280;">Published by: ${p.publishedBy ? p.publishedBy.name : 'Father Alex'} • ${new Date(p.publishedAt).toLocaleDateString()}</div>
          </div>
          <span class="studio-card-badge studio-badge-published">PUBLISHED</span>
        </div>
        <div class="studio-card-actions">
          <button class="secondary-btn btn-sm" onclick="window.KOINONIA_GAME.handlePreviewStudioDraft('${p.draftId}')">👁️ View Details</button>
        </div>
      </div>
    `).join('');
  }
  ...
  function renderStudioMedia() {
    if (typeof document === 'undefined') return;
    const container = document.getElementById('studio-media-list');
    if (!container) return;

    if (typeof KoinoniaStudio === 'undefined' || !KoinoniaStudio.DEMO_MEDIA_LIBRARY) return;

    const library = KoinoniaStudio.DEMO_MEDIA_LIBRARY;
    container.innerHTML = `
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
        ${library.map(m => `
          <div style="background: #FFFFFF; border: 1px solid var(--border-light); border-radius: 10px; padding: 10px; text-align: center;">
            <div style="font-size: 2rem; margin-bottom: 6px;">🖼️</div>
            <div style="font-weight: 700; font-size: 0.78rem; color: var(--brand-charcoal); line-height: 1.2;">${m.title}</div>
            <div style="font-size: 0.65rem; color: #6B7280; margin-top: 2px;">${m.mimeType} • ${m.aspectRatio}</div>
            <div style="font-size: 0.62rem; color: #9CA3AF; margin-top: 1px;">${m.filename}</div>
          </div>
        `).join('')}
      </div>
    `;
  ...
  function renderStudioAudit() {
    if (typeof document === 'undefined') return;
    const container = document.getElementById('studio-audit-list');
    if (!container) return;

    if (typeof KoinoniaStudio === 'undefined' || !KoinoniaStudio.store) return;

    const logs = KoinoniaStudio.store.getAuditLogs() || [];
    if (logs.length === 0) {
      container.innerHTML = '<div class="studio-empty-state"><div class="studio-empty-text">No audit records logged yet.</div></div>';
      return;
    }

    container.innerHTML = logs.map(l => `
      <div style="background: #FFFFFF; border: 1px solid var(--border-light); border-radius: 8px; padding: 8px 12px; margin-bottom: 8px; font-size: 0.75rem;">
        <div style="display: flex; justify-content: space-between; font-weight: 700;">
          <span style="color: var(--brand-burgundy);">${l.action}</span>
          <span style="color: #9CA3AF;">${new Date(l.timestamp).toLocaleTimeString()}</span>
        </div>
        <div style="color: var(--brand-charcoal); margin-top: 2px;">
          Actor: <strong>${l.actor ? l.actor.name : 'Unknown'}</strong> (${l.actor ? l.actor.role : 'MEMBER'})
        </div>
        <div style="color: #4B5563; font-style: italic; margin-top: 2px;">"${l.title || 'Untitled'}"</div>
      </div>
    `).join('');
  }
  ...
  function applyBetaIdentity(identity) {
    ...
    // Sync Growth Profile state (Phase 0.23J)
    let growthProfile = null;
    if (typeof KoinoniaGrowth !== 'undefined' && typeof KoinoniaGrowth.getGrowthProfile === 'function') {
      growthProfile = KoinoniaGrowth.getGrowthProfile(identity);
    } else if (typeof BetaIdentityProvider !== 'undefined' && typeof BetaIdentityProvider.getGrowthProfile === 'function') {
      growthProfile = BetaIdentityProvider.getGrowthProfile(identity);
    }

    if (growthProfile && growthProfile.progression) {
      state.lp = growthProfile.progression.lifePoints;
      state.charXp = growthProfile.progression.xp;
      state.charLevel = growthProfile.progression.level;
      state.charXpMax = growthProfile.progression.nextLevelXp;
      if (Array.isArray(growthProfile.milestones)) {
        state.unlockedMilestones = growthProfile.milestones.map(m => m.id);
      }
    }

    if (typeof updateLpDisplay === 'function') updateLpDisplay();
    if (typeof updateCharacterProgressDisplays === 'function') updateCharacterProgressDisplays();
    if (typeof updateSkillDisplays === 'function') updateSkillDisplays();
    if (typeof updateProfileMemberDetails === 'function') updateProfileMemberDetails();
    if (typeof renderMilestones === 'function') renderMilestones();
    if (typeof renderJourneySummary === 'function') renderJourneySummary();
    ...
    if (acctStudio) {
      const canAccess = Boolean(
        (identity.capabilities && identity.capabilities.canAccessStudio) ||
  ...
  function answerFaithQuestSample(choiceIndex) {
    ...
    if (choiceIndex === 1) {
      ...
      let resultText = '';
      if (sc && sc.capabilities && sc.capabilities.remote) {
        const isUnavailable = !sc.capabilities.readsEnabled;
        if (isUnavailable) {
          resultText = '✓ Correct! "To glorify God, and to enjoy Him forever." (SHARED CORE CONFIG UNAVAILABLE. Progress writes are disabled.)';
          showToast('⚠️ Action not saved. SHARED CORE CONFIG UNAVAILABLE. Progress writes are disabled.');
        } else {
          resultText = '✓ Correct! "To glorify God, and to enjoy Him forever." (Shared Core is in Read-Only mock mode. No Life Points or progress written.)';
          showToast('ℹ️ Action not saved. Shared progress was not changed because Shared Core is running in Read-Only development mode.');
        }
        if (feedback) {
      ...
      let authRes = null;
      if (typeof KoinoniaRewardAuthority !== 'undefined' && typeof KoinoniaRewardAuthority.recordArcadeReward === 'function') {
        authRes = KoinoniaRewardAuthority.recordArcadeReward(activeMemberId, 'faithquest_catechism', {
          activityName: 'Faith Quest Catechism Practice'
        });
      }
      ...
      if (authRes && authRes.success) {
        ...
        updateLpDisplay();
        updateCharacterProgressDisplays();
        resultText = authRes.alreadyCompleted
          ? '✓ Correct! "To glorify God, and to enjoy Him forever." (Practice reward already credited today. Current balance: ' + state.lp + ' LP)'
          : '✓ Correct! "To glorify God, and to enjoy Him forever." Awarded +' + authRes.rewardsGranted.lifePoints + ' LP and +' + authRes.rewardsGranted.xp + ' Char XP! New balance: ' + state.lp + ' LP.';
      } else if (sc) {
  ...
  function playArcadeDemo(gameId = 'arcade_slingshot') {
    ...
    if (sc && sc.capabilities && sc.capabilities.remote) {
      const isUnavailable = !sc.capabilities.readsEnabled;
      if (isUnavailable) {
        resultText = '🎯 Bullseye! (SHARED CORE CONFIG UNAVAILABLE. Progress writes are disabled.)';
        showToast('⚠️ Action not saved. SHARED CORE CONFIG UNAVAILABLE. Progress writes are disabled.');
      } else {
        resultText = '🎯 Bullseye! (Shared Core is in Read-Only mock mode. No Life Points or progress written.)';
        showToast('ℹ️ Action not saved. Shared progress was not changed because Shared Core is running in Read-Only development mode.');
      }
      if (feedback) {
        feedback.style.display = 'block';
    ...
    let authRes = null;
    if (typeof KoinoniaRewardAuthority !== 'undefined' && typeof KoinoniaRewardAuthority.recordArcadeReward === 'function') {
      authRes = KoinoniaRewardAuthority.recordArcadeReward(activeMemberId, gameId || 'arcade_slingshot', {
        activityName: "David's Slingshot Arcade Challenge"
      });
    }
    ...
    if (authRes && authRes.success) {
      ...
      if (authRes.alreadyCompleted) {
        resultText = '🎯 Bullseye hit! (Daily +5 LP already collected today. Current balance: ' + state.lp + ' LP)';
      } else {
        resultText = '🎯 Bullseye! Goliath shield struck! Awarded +' + authRes.rewardsGranted.lifePoints + ' LP and +' + authRes.rewardsGranted.xp + ' Char XP! New balance: ' + state.lp + ' LP.';
      }
    } else if (sc) {
  ...
  function initSharedCoreDevHarness() {
    if (typeof window === 'undefined' || typeof document === 'undefined') return;

    const rc = window.__KOINONIA_RUNTIME_CONFIG__;
    const isMock = rc && rc.sharedCoreMode === 'mock';
    const isUnavailable = !rc || rc.sharedCoreMode === 'unavailable' || Boolean(rc.configLoadFailed);

    const banner = document.getElementById('shared-core-dev-banner');
    const badge = document.getElementById('shared-core-dev-badge');
    const modal = document.getElementById('shared-core-diagnostics-modal');
    const closeBtn = document.getElementById('btn-close-shared-core-diag');

    if (!isMock && !isUnavailable) {
    ...
    // When unavailable (config failed to load or runtime config missing):
    if (isUnavailable) {
      if (banner) {
        banner.classList.remove('hidden');
        banner.classList.add('banner-unavailable');
      }
      if (badge) {
        badge.classList.remove('hidden');
        badge.classList.add('badge-unavailable');
        const textEl = badge.querySelector('.shared-core-dev-badge-text');
        if (textEl) textEl.textContent = 'SHARED CORE CONFIG UNAVAILABLE • Progress writes disabled';
      }
      showToast('⚠️ SHARED CORE CONFIG UNAVAILABLE. Progress writes are disabled.');
    } else {
      // Mock mode enabled by server environment
      if (banner) {
        banner.classList.remove('hidden');
        banner.classList.remove('banner-unavailable');
      }
      if (badge) {
        badge.classList.remove('hidden');
        badge.classList.remove('badge-unavailable');
        const textEl = badge.querySelector('.shared-core-dev-badge-text');
        if (textEl) textEl.textContent = 'SHARED CORE DEV: MOCK • READ ONLY';
      }
    }

    const provider = (typeof window.getSharedCoreProvider === 'function')
      ? window.getSharedCoreProvider()
      : (window.SharedCore || null);

    const AsyncControllerClass = window.SharedCoreAsyncController || (typeof require !== 'undefined' ? require('./data/shared_core_provider.js').SharedCoreAsyncController : null);
    if (!AsyncControllerClass) return;

    const controller = new AsyncControllerClass({ provider });
    window.sharedCoreAsyncController = controller;

    function updateDiagnosticsModal(diagState) {
      const pEl = document.getElementById('sc-diag-provider');
      const mEl = document.getElementById('sc-diag-mode');
      const sEl = document.getElementById('sc-diag-status');
      const tEl = document.getElementById('sc-diag-last-time');
      const eEl = document.getElementById('sc-diag-last-error');
      const memEl = document.getElementById('sc-diag-member');
      const lpEl = document.getElementById('sc-diag-lp');
      const qEl = document.getElementById('sc-diag-quests');
      const evEl = document.getElementById('sc-diag-events');
      const cfEl = document.getElementById('sc-diag-campfires');
      const minEl = document.getElementById('sc-diag-ministries');
      const mlsEl = document.getElementById('sc-diag-milestones');
      const gEl = document.getElementById('sc-diag-growth');

      if (pEl) pEl.textContent = diagState.isRemote ? 'RemoteSharedCoreProvider' : 'LocalSharedCoreProvider';
      if (mEl) {
        if (isUnavailable) {
          mEl.textContent = 'Unavailable (Config Load Failed)';
        } else {
          mEl.textContent = diagState.isRemote ? 'Mock / Read Only' : 'Local';
        }
      }
      if (sEl) {
        if (diagState.status === 'connected') {
          sEl.textContent = 'Connected';
          sEl.style.color = '#16A34A';
        } else if (diagState.status === 'loading') {
          sEl.textContent = 'Loading...';
          sEl.style.color = '#0284C7';
        } else {
          sEl.textContent = 'Unavailable';
          sEl.style.color = '#DC2626';
        }
      }
      if (tEl) tEl.textContent = diagState.lastSuccessTime || diagState.lastRequestTime || '-';
      if (eEl) {
        eEl.textContent = diagState.lastErrorCode || 'None';
        eEl.style.color = diagState.lastErrorCode ? '#DC2626' : '#16A34A';
      }
      if (memEl) memEl.textContent = (diagState.cachedData.member && diagState.cachedData.member.name) || '-';
      if (lpEl) {
        const lp = diagState.cachedData.lifePoints;
        lpEl.textContent = (lp && lp.balance !== undefined) ? `${lp.balance} LP` : '-';
      }
      if (qEl) {
        const q = diagState.cachedData.quests;
        qEl.textContent = Array.isArray(q) ? `${q.length} quests` : '-';
      }
      if (evEl) {
        const ev = diagState.cachedData.events;
        evEl.textContent = Array.isArray(ev) ? `${ev.length} events` : '-';
      }
      if (cfEl) {
        const cf = diagState.cachedData.campfires;
        cfEl.textContent = Array.isArray(cf) ? `${cf.length} campfires` : '-';
      }
      if (minEl) {
        const min = diagState.cachedData.ministries;
        minEl.textContent = Array.isArray(min) ? `${min.length} ministries` : '-';
      }
      if (mlsEl) {
        const mls = diagState.cachedData.milestones;
        mlsEl.textContent = Array.isArray(mls) ? `${mls.length} milestones` : '-';
      }
      if (gEl) {
        const gp = diagState.cachedData.growthProgress;
        gEl.textContent = (gp && gp.level !== undefined) ? `Level ${gp.level}` : '-';
      }
    }

    controller.subscribe(updateDiagnosticsModal);

    function openDiagModal() {
      if (modal) {
        modal.classList.remove('hidden');
        updateDiagnosticsModal(controller.getState());
      }
    }
    ...
    if (badge) {
      badge.addEventListener('click', openDiagModal);
    ...
    // Trigger non-blocking representative read sync
    controller.syncAllRepresentativeReads().then(res => {
      if (!res.allSuccess && res.status === 'unavailable') {
        if (isUnavailable) {
          showToast('⚠️ SHARED CORE CONFIG UNAVAILABLE. Progress writes are disabled.');
        } else {
          showToast('Shared Core unavailable. Your local KOINONIA world is still running. No shared progress was changed.');
        }
      }
    }).catch(() => {
      if (isUnavailable) {
  ...
  function init() {
    // Phase 0.23J: Harmonize active persona and growth profile on initialization
    if (typeof BetaIdentityProvider !== 'undefined') {
      applyBetaIdentity(BetaIdentityProvider.getCurrentIdentity());
    }
    ...
    // Phase 0.23D: Initialize Shared Core Development Harness (Status Badge & Diagnostics)
    initSharedCoreDevHarness();
  ...
  root.KOINONIA_GAME = {
    ...
    // Phase 0.23F: Studio Dashboard & Governance UX
    openAdminStudio,
    closeAdminStudio,
    getStudioUserIdentity,
    updateStudioIdentityBanner,
    updateStudioCounts,
    switchStudioView,
    launchWizardForTemplate,
    returnFromWizardToHome,
    updateWizardUI,
    advanceWizard,
    rewindWizard,
    handleSaveStudioDraft,
    handleSubmitStudioDraftForReview,
    handleApproveStudioDraft,
    handleReturnStudioDraftForChanges,
    openReturnForChangesModal,
    closeReturnForChangesModal,
    confirmReturnForChanges,
    handlePublishStudioDraft,
    ...
    renderStudioDrafts,
    renderStudioSubmitted,
    renderStudioReviewQueue,
    renderStudioPublished,
    renderStudioMedia,
    renderStudioAudit,
    ...
    updateSettingsProfileSummary,
    updateProfileMemberDetails,
    getGrowthProfile: (identity) => (typeof KoinoniaGrowth !== 'undefined' ? KoinoniaGrowth.getGrowthProfile(identity) : null),
    getSharedCore: () => (typeof window !== 'undefined' ? window.SharedCore : null),
