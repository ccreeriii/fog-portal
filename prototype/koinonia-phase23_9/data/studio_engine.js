  ...
  class StudioPlaceStore {
    ...
    createPlace(data = {}, actor = {}) {
      ...
      const newPlace = {
        ...
        status: 'ACTIVE',
        createdBy: {
          id: actor.memberId || actor.id || 'unknown_user',
          name: actor.displayName || actor.name || 'Anonymous Leader',
          role
        },
        createdAt: Date.now()
      };
  ...
  class StudioStore {
    ...
    persist() {
      ...
      try {
        ...
        storage.setItem(STORAGE_KEYS.AUDIT, JSON.stringify(this.inMemoryAudit.slice(0, 100)));
      } catch (err) {
    ...
    canPerform(roleOrIdentity, action) {
      if (roleOrIdentity && typeof roleOrIdentity === 'object') {
        const caps = roleOrIdentity.capabilities;
        if (caps) {
          if (action === 'VIEW_STUDIO') return Boolean(caps.canAccessStudio);
          if (action === 'CREATE_DRAFT' || action === 'UPDATE_DRAFT' || action === 'REQUEST_REVIEW' ||
              action === 'DUPLICATE_DRAFT' || action === 'ARCHIVE_DRAFT' || action === 'DELETE_DRAFT' ||
              action === 'PREVIEW' || action === 'CREATE_PLACE') {
            return Boolean(caps.canAuthorContent);
          }
          if (action === 'APPROVE' || action === 'RETURN_FOR_CHANGES') return Boolean(caps.canReviewContent);
          if (action === 'PUBLISH') return Boolean(caps.canPublishContent);
        }
        roleOrIdentity = roleOrIdentity.role;
      }
      const r = String(roleOrIdentity || '').toUpperCase();
    ...
    logAudit(actor, action, draftId, title, details = {}) {
      const act = actor || {};
      const actorId = act.memberId || act.id || 'unknown_user';
      const actorName = act.displayName || act.name || 'Anonymous Leader';
      const actorRole = (act.role || ROLES.MEMBER).toUpperCase();
      const entry = {
    ...
    createDraft(templateType, initialData = {}, actor = {}) {
      ...
      const newDraft = {
        ...
        createdBy: {
          id: actor.memberId || actor.id || 'admin_user',
          name: actor.displayName || actor.name || 'Admin Leader',
    ...
    updateDraft(draftId, updatedData, actor = {}) {
      ...
      // If in review or approved and edited, revert to DRAFT so changes require re-review
      if (draft.status === DRAFT_STATUS.READY_FOR_REVIEW || draft.status === DRAFT_STATUS.APPROVED) {
    ...
    requestReview(draftId, actor = {}) {
      ...
      // Guard: Cannot submit if already submitted for review (duplicate submit prevention)
      if (draft.status === DRAFT_STATUS.READY_FOR_REVIEW) {
        throw new Error(`Draft '${draftId}' is already submitted for review.`);
      }

      // Guard: Only DRAFT state can transition to READY_FOR_REVIEW
      if (draft.status !== DRAFT_STATUS.DRAFT) {
        throw new Error(`Cannot submit draft in '${draft.status}' state for review.`);
      }
    ...
    approveDraft(draftId, actor = {}) {
      ...
      // Guard: Strictly ONLY READY_FOR_REVIEW can be approved
      if (draft.status !== DRAFT_STATUS.READY_FOR_REVIEW) {
        throw new Error(`Cannot approve draft in '${draft.status}' state. Content must be READY_FOR_REVIEW before approval.`);
      }
    ...
    returnForChanges(draftId, actor = {}, notes = '') {
      if (!this.canPerform(actor.role, 'APPROVE')) {
        throw new Error(`Unauthorized: Only SUPERADMIN leaders can return Studio content for changes.`);
      }

      const draft = this.inMemoryDrafts.get(draftId);
      if (!draft) throw new Error(`Draft '${draftId}' not found.`);

      // Guard: Only READY_FOR_REVIEW or APPROVED items can be returned for changes
      if (draft.status !== DRAFT_STATUS.READY_FOR_REVIEW && draft.status !== DRAFT_STATUS.APPROVED) {
        throw new Error(`Cannot return draft in '${draft.status}' state for changes.`);
      }

      const previousStatus = draft.status;
      draft.status = DRAFT_STATUS.DRAFT;
      draft.updatedAt = Date.now();
      draft.returnedAt = Date.now();
      draft.returnedBy = {
        id: actor.memberId || actor.id || 'superadmin_user',
        name: actor.displayName || actor.name || 'Father Alex',
        role: (actor.role || ROLES.SUPERADMIN).toUpperCase()
      };
      draft.reviewNotes = notes ? String(notes).trim() : 'Changes requested by leadership.';

      this.inMemoryDrafts.set(draftId, draft);
      this.persist();
      this.logAudit(actor, AUDIT_ACTIONS.RETURNED_FOR_CHANGES, draftId, draft.title, {
    ...
    publishDraft(draftId, actor = {}) {
      ...
      // Notify Runtime Content Registry if present
      if (typeof window !== 'undefined' && window.KoinoniaContentRegistry && typeof window.KoinoniaContentRegistry.refresh === 'function') {
        window.KoinoniaContentRegistry.refresh();
      } else if (typeof global !== 'undefined' && global.KoinoniaContentRegistry && typeof global.KoinoniaContentRegistry.refresh === 'function') {
        global.KoinoniaContentRegistry.refresh();
      }
