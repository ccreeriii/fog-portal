...
(function () {
  ...
  // ============================================================
  // 19. KOINONIA STUDIO DASHBOARD & GOVERNANCE ENGINE (PHASE 0.23E)
  // ============================================================
  function getStudioUserIdentity() {
    ...
    return {
      ...
      role: (state.role || 'MEMBER').toUpperCase(),
      avatar: '🧑',
  ...
  function updateStudioCounts() {
    ...
    setText('nav-count-review', submittedCount);
  ...
  function handleReturnStudioDraftForChanges(draftId) {
    const identity = getStudioUserIdentity();
    if (identity.role !== 'SUPERADMIN') {
    ...
    try {
      ...
      if (draft) {
        ...
        KoinoniaStudio.store.logAudit(actor, 'RETURNED_FOR_CHANGES', targetId, draft.title);
      }
      updateStudioCounts();
      showToast('↩️ Returned to author for changes.');
      if (state.studioView === 'review') renderStudioReviewQueue();
    } catch (err) {
    ...
    container.innerHTML = drafts.map(d => `
      <div class="studio-card-item">
        <div class="studio-card-header">
          ...
          <span class="studio-card-badge studio-badge-draft">DRAFT</span>
        </div>
  ...
  function resumeDraftInWizard(draftId) {
    ...
    const nameInput = document.getElementById('wizard-place-name');
    if (nameInput && draft.data && draft.data.placeName) nameInput.value = draft.data.placeName;
      ...
      <div class="studio-card-item">
        ...
        <div class="studio-card-actions">
          ...
          ${isSuper ? `
            ...
            <button class="secondary-btn btn-sm" onclick="window.KOINONIA_GAME.handleReturnStudioDraftForChanges('${d.id}')">↩️ Return Edits</button>
          ` : ''}
        </div>
      </div>
    `).join('');
  ...
  root.KOINONIA_GAME = {
    ...
    // Phase 0.23E: Studio Dashboard & Governance UX
    openAdminStudio,
    closeAdminStudio,
