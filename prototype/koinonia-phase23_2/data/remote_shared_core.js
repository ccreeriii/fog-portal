...
(function(root) {
  ...
  class SharedCoreHttpClient {
    ...
    isValidSharedCoreEndpoint(endpoint) {
      ...
      // Reject absolute URLs with schemes (http:, https:, ftp:, etc.) or schema-relative URLs (//)
      if (/^[a-zA-Z][a-zA-Z0-9+.-]*:/.test(trimmed) || trimmed.startsWith('//') || trimmed.startsWith('\\\\')) {
        return false;
      }

      // Reject path traversal (e.g., .., %2e%2e)
      if (trimmed.includes('..')) {
        return false;
      }
      try {
      ...
      // Must start strictly with canonical prefix
    ...
    async request(method, endpoint, body = null, options = {}) {
      ...
      const authHeader = options.authHeader || this.authHeader;
      if (authHeader) {
        headers['Authorization'] = authHeader;
      }

      // CSRF extension point
      const csrfToken = options.csrfToken || this.csrfToken;
      if (csrfToken) {
      ...
      try {
        ...
        const isTimeout = err && (err.name === 'AbortError' || String(err.message).toLowerCase().includes('timeout') || String(err.message).toLowerCase().includes('abort'));
        if (isTimeout) {
          return createFailClosedResult(
            SHARED_CORE_ERRORS.TIMEOUT,
      ...
      // Status code mapping for non-2xx responses
      if (!response.ok) {
        const status = response.status;
        const errorMsg = (parsedData && (parsedData.message || parsedData.error)) || `HTTP ${status} ${response.statusText || ''}`.trim();
        if (status === 401) {
          return createFailClosedResult(SHARED_CORE_ERRORS.UNAUTHORIZED, errorMsg, { status, data: parsedData });
        }
        if (status === 403) {
          return createFailClosedResult(SHARED_CORE_ERRORS.FORBIDDEN, errorMsg, { status, data: parsedData });
        }
        if (status === 404) {
          return createFailClosedResult(SHARED_CORE_ERRORS.NOT_FOUND, errorMsg, { status, data: parsedData });
        }
        if (status === 409) {
          return createFailClosedResult(SHARED_CORE_ERRORS.CONFLICT, errorMsg, { status, data: parsedData });
        }
        if (status === 422) {
          return createFailClosedResult(SHARED_CORE_ERRORS.VALIDATION_ERROR, errorMsg, { status, data: parsedData });
        }
        if (status === 429) {
          return createFailClosedResult(SHARED_CORE_ERRORS.RATE_LIMITED, errorMsg, { status, data: parsedData });
        }
        if (status >= 500 && status <= 599) {
          return createFailClosedResult(SHARED_CORE_ERRORS.SERVER_ERROR, errorMsg, { status, data: parsedData });
        }
        return createFailClosedResult(SHARED_CORE_ERRORS.NETWORK_ERROR, errorMsg, { status, data: parsedData });
      }

      // For 2xx responses, verify application/json Content-Type and valid JSON
      const contentType = (response.headers && typeof response.headers.get === 'function')
        ? (response.headers.get('content-type') || '')
        : '';

      if (!contentType.includes('application/json') || parseError) {
        return createFailClosedResult(
          SHARED_CORE_ERRORS.MALFORMED_RESPONSE,
          `Malformed or non-JSON response from Shared Core (Content-Type: ${contentType}).`
        );
      }
  ...
  class RemoteSharedCoreProvider {
    ...
    resetToBaseline() {
      return createFailClosedResult(
        SHARED_CORE_ERRORS.OPERATION_UNSUPPORTED,
        'resetToBaseline is available only for the local prototype provider.'
      );
    }
