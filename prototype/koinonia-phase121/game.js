/**
 * KOINONIA — PHASE 0.12.1 GAME ENGINE
 * Real Phone Device-Classification & Portrait Gameplay Fix
 *
 * Key Architecture:
 * 1. Robust Short-Side Device Classification:
 *    - Uses visualViewport / inner dimensions: shortSide = Math.min(vw, vh), longSide = Math.max(vw, vh).
 *    - All phones (iPhone 8+, iPhone 12/13/14 Pro, Android, etc.) are reliably identified by shortSide <= 600px.
 *    - Eliminates flawed width-only breakpoints (e.g. 767px) that broke landscape phones.
 * 2. Central Responsive State & Root Classes:
 *    - Single source of truth in JavaScript drives explicit root classes:
 *      .device-phone | .device-tablet | .device-desktop
 *      .orientation-portrait | .orientation-landscape
 *      .active-game | .app-shell
 * 3. Phone Portrait Guarantee:
 *    - App shell browsing (Home Play Card, Quests, World, Journey, Me).
 *    - Tapping [ 🌿 ENTER WORLD ] directly activates full-stage portrait RPG exploration.
 *    - [ ✕ EXIT WORLD ] header button returns to Home card.
 * 4. Phone Landscape Guarantee:
 *    - NO active RPG map rendered on phone landscape.
 *    - Safely pauses game and displays branded Landscape Companion Screen:
 *      "Your journey continues in portrait mode. Turn your phone upright to play."
 *      Glanceable cards: Current Place, Active Quest, Life Points, Today's Focus.
 *    - Auto-resumes at exact same coordinates on return to portrait.
 * 5. Viewport Calibration & Safari visualViewport:
 *    - calibrateGameViewport() verifies non-zero stage dimensions before camera computation.
 *    - Dynamically absorbs Safari address bar expansion/collapse via visualViewport listeners.
 * 6. Diagnostics HUD (?debug=1):
 *    - Real-time display of vw, vh, shortSide, longSide, deviceClass, orientation, activeGame, stage, canvas backing, zoom.
...
(function () {
  ...
  function updateDebugHud() {
    ...
    hud.innerHTML = `
      <strong>KOINONIA Phase 0.12.1 HUD</strong><br>
      ...
      LP: <b>${state.lp}</b> | Quest: <b>${state.questStatus}</b>
    `;
  }
