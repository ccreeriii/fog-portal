...
(function () {
  ...
  const SAVE_STORAGE_KEY = 'koinonia.phase13.save';
  ...
  function updateDebugHud() {
    ...
    hud.innerHTML = `
      <strong>KOINONIA Phase 0.13 Engine HUD</strong><br>
      ...
      Save Key: <code>${SAVE_STORAGE_KEY}</code> (v${state.saveVersion})<br>
      Last Saved: <small>${state.lastSaveTime || 'Initial State'}</small><br>
  ...
  // ============================================================
  // 15. LOCAL SAVE & RESTORE ENGINE (koinonia.phase13.save)
  // ============================================================
  let saveDebounceTimer = null;
  function queuePositionSave() {
    if (saveDebounceTimer) clearTimeout(saveDebounceTimer);
    saveDebounceTimer = setTimeout(saveToStorage, 600);
  }
  ...
  function saveToStorage() {
    ...
    try {
      const saveData = {
        ...
        unlockedPlaces: [...state.unlockedPlaces],
  ...
  function loadFromStorage() {
    ...
    try {
      // Check query string for developer reset ?reset=1
      ...
      if (parsed.activePlaceId) state.activePlaceId = parsed.activePlaceId;
      if (parsed.spawnId) state.spawnId = parsed.spawnId;

      if (parsed.avatar) {
        if (typeof parsed.avatar.x === 'number') state.avatar.x = parsed.avatar.x;
        if (typeof parsed.avatar.y === 'number') state.avatar.y = parsed.avatar.y;
        if (parsed.avatar.dir) state.avatar.dir = parsed.avatar.dir;
      }

      if (typeof parsed.lp === 'number') state.lp = parsed.lp;
      ...
      if (parsed.skills) state.skills = Object.assign(state.skills, parsed.skills);
      if (parsed.gardenState) state.gardenState = parsed.gardenState;
      ...
      if (Array.isArray(parsed.unlockedPlaces)) state.unlockedPlaces = parsed.unlockedPlaces;
      if (parsed.questStatus) state.questStatus = parsed.questStatus;
      if (typeof parsed.rewardClaimed === 'boolean') state.rewardClaimed = parsed.rewardClaimed;
      if (parsed.currentObjective) state.currentObjective = parsed.currentObjective;
      if (parsed.reflectionText) state.reflectionText = parsed.reflectionText;
