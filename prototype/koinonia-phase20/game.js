...
(function () {
  ...
  const SAVE_STORAGE_KEY = 'koinonia.phase20.save';
    ...
    if (!list || list.length === 0) {
      container.innerHTML = `
        <div style="text-align: center; padding: 28px 16px; color: var(--brand-charcoal); opacity: 0.85;">
          ...
          <div style="font-size: 0.8rem; line-height: 1.4;">Complete quests, join events, explore places, and try Fit Quests to create memories.</div>
        </div>
      `;
      return;
    }
  ...
  function completeRealWorldQuest() {
    ...
    // Phase 0.19: Create Real-World Fit Quest Completed Memory
    ...
    if (mDataForRw && typeof mDataForRw.addMemoryIfNew === 'function') {
      ...
      mDataForRw.addMemoryIfNew(state, {
        ...
        title: rwChallenge ? rwChallenge.title : 'Real-World Fit Quest',
  ...
  function setupEventListeners() {
    ...
    if (typeof window !== 'undefined' && window.location && window.location.search) {
      try {
        ...
        } else if (openParam === 'circles' || urlParams.get('target') === 'circles') {
          setTimeout(() => openQuestCirclesModal(), 300);
  ...
  function setCircleRole(newRole) {
    state.circleRole = (newRole === 'LEADER' || newRole === 'ADMIN') ? 'LEADER' : 'MEMBER';
