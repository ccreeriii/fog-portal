...
(function () {
  ...
  const SAVE_STORAGE_KEY = 'koinonia.phase15.save';
  ...
  function updateDebugHud(force = false) {
    ...
    hud.innerHTML = `
      <strong>KOINONIA Phase 0.15 Character Growth & RPG HUD</strong><br>
      ...
      Milestones: <b>${unlockedMCount} / 11 Unlocked</b><br>
  ...
  function evaluateCharacterProgression(reason = 'update') {
    ...
    const growthKeys = pData?.GROWTH_AREAS ? Object.keys(pData.GROWTH_AREAS) : ['stewardship', 'responsibility', 'service', 'reflection', 'discipline', 'teamwork'];
    ...
    if (leveledUp && levelDef?.unlockedPerk) {
      state.unlockedPerks = state.unlockedPerks || [];
      if (!state.unlockedPerks.includes(levelDef.unlockedPerk)) {
  ...
  function renderGrowthAreas() {
    ...
    if (!container || !pData || !pData.GROWTH_AREAS) return;

    let html = '';
    for (const [key, area] of Object.entries(pData.GROWTH_AREAS)) {
  ...
  function renderMilestones() {
    ...
    if (!container || !pData || !pData.MILESTONES) return;

    let html = '';
    pData.MILESTONES.forEach(m => {
