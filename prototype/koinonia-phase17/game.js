...
(function () {
  ...
  const SAVE_STORAGE_KEY = 'koinonia.phase17.save';
  ...
  function renderSportsHubWorld(ctx) {
    ...
    // Running track perimeter ring (Terracotta / Red brick)
    ...
    // Basketball Half-Court (Center-West)
    const courtX = 5 * 32;
    const courtY = 3 * 32;
    const courtW = 14 * 32;
    const courtH = 10 * 32;
    ...
    ctx.fillRect(10 * 32, courtY, 4 * 32, 5 * 32);
    ctx.strokeStyle = '#FFFFFF';
    ctx.strokeRect(10 * 32, courtY, 4 * 32, 5 * 32);
    ctx.beginPath();
    ctx.arc(12 * 32, courtY + 5 * 32, 2 * 32, 0, Math.PI);
    ctx.stroke();

    // 3-Point Arc
    ctx.beginPath();
    ctx.arc(12 * 32, courtY + 16, 5 * 32, 0, Math.PI);
    ctx.stroke();

    // Hoop & Backboard (North rim)
    ctx.fillStyle = '#FFFFFF';
    ctx.fillRect(11 * 32, courtY + 4, 2 * 32, 6);
    ctx.fillStyle = '#E65100';
    ctx.beginPath();
    ctx.arc(12 * 32, courtY + 14, 8, 0, Math.PI * 2);
    ctx.stroke();
    ctx.font = '16px sans-serif';
    ctx.fillText('🏀', 12 * 32 - 8, courtY + 18);
    ...
    ctx.fillText('BLEACHERS', 20 * 32, 9 * 32 + 16);

    // Hydration Station (East)
    ctx.fillStyle = '#0288D1';
    ctx.fillRect(20 * 32, 11 * 32, 32, 32);
    ctx.font = '18px sans-serif';
    ctx.fillText('💧', 20 * 32 + 6, 11 * 32 + 22);
    ctx.fillStyle = '#0277BD';
    ctx.font = 'bold 8px "Clear Sans", sans-serif';
    ctx.fillText('HYDRATION', 20 * 32 - 4, 12 * 32 + 14);

    // Activity Board (West)
    ctx.fillStyle = '#78350F';
    ctx.fillRect(2 * 32, 5 * 32, 28, 36);
    ctx.fillStyle = '#FDE68A';
    ctx.fillRect(2 * 32 + 3, 5 * 32 + 3, 22, 30);
    ctx.font = '16px sans-serif';
    ctx.fillText('📋', 2 * 32 + 6, 5 * 32 + 22);
  ...
  function updateDebugHud(force = false) {
    ...
    hud.innerHTML = `
      <strong>KOINONIA Phase 0.17 Campaigns & Events Engine HUD</strong><br>
    ...
    if (pId === 'home') {
      ...
      return [
        ...
        },
        {
          ...
          radius: 2.2,
          type: 'activity_point',
          getPrompt: () => 'READ — Sports Activity Board [E]',
          onInteract: () => openQuestsTabModal()
        },
        {
          id: 'sports_hoop',
          ...
          radius: 2.0,
          type: 'object',
          getPrompt: () => 'SHOOT — Basketball Free Throw [E]',
          onInteract: () => {
            triggerEmote('🏀');
            showToast('🏀 *Swish!* Great shot! Physical health honors God.');
          }
        },
        {
      ...
      return [
        ...
        },
        {
          ...
          radius: 2.2,
          type: 'notice_board',
          getPrompt: () => 'VIEW — Community Needs Board [E]',
          onInteract: () => openQuestsTabModal()
        },
        {
          ...
          radius: 2.2,
          type: 'activity_point',
          getPrompt: () => 'PACK — Help Pack Care Packages [E]',
          onInteract: () => {
            triggerEmote('📦');
        ...
        },
        {
          ...
          radius: 2.0,
          type: 'object',
          getPrompt: () => 'ORGANIZE — Food Pantry Shelves [E]',
          onInteract: () => {
            triggerEmote('🥫');
            showToast('🥫 Pantry supplies neatly sorted and ready to distribute to neighbors.');
          }
        },
        {
          id: 'outreach_exit',
  ...
  function openDialogueModal(npcId = 'barnaby') {
    ...
    if (npcId === 'barnaby') {
      ...
      if (isFirst) {
        if (textEl) textEl.textContent = '"Hey there, Alex! Welcome to the Sports Hub! Your body is a temple—let\'s build strength, discipline, and teamwork through active play."';
      } else {
        if (textEl) textEl.textContent = '"Sports challenges are coming in the next season! Remember: true sportsmanship means encouraging your teammates, win or lose."';
      }

      if (actionBtn) {
        actionBtn.style.display = 'inline-flex';
        actionBtn.querySelector('span').textContent = 'SHOOT FREE THROW';
        actionBtn.onclick = () => {
          closeDialogueModal();
          triggerEmote('🏀');
          showToast('🏀 *Swish!* Great shot! Physical health honors God.');
  ...
  function loadFromStorage() {
    ...
    try {
      ...
      const isVersionMatch = parsed && (parsed.version === SAVE_VERSION || parsed.saveVersion === SAVE_VERSION || parsed.version === '0.17' || parsed.saveVersion === '0.17' || parsed.version === 1);
        ...
        return `
          <div class="team-chip" style="cursor: pointer; padding: 12px; opacity: ${opacity}; display: flex; align-items: center; justify-content: space-between; border-radius: var(--radius-md); border: 1px solid ${isCurrent ? 'var(--brand-fire-orange)' : 'rgba(0,0,0,0.08)'}; margin-bottom: 8px;" onclick="window.KOINONIA_GAME.handlePlaceSelect('${p.id}')">
            <div style="display: flex; align-items: center; gap: 12px;">
              ...
              <div>
                <div style="font-weight: 800; font-size: 0.95rem; color: var(--brand-burgundy); display: flex; align-items: center;">${p.name} ${liveEventBadge}</div>
