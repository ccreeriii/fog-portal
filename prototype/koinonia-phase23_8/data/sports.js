/**
 * KOINONIA — PHASE 0.18 / 0.20.1 SPORTS & FAITH QUEST DATA STORE
 * Architecture: Sports Registry, Faith Quest Challenge Catalog, Scoring Engine,
...
(function () {
  ...
  // ============================================================
  // 3. FAITH QUEST CHALLENGE CATALOG
  // ============================================================
  const CHALLENGES = {
    // ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // REAL-WORLD FAITH QUESTS (Honor / Trust Verification)
    // ------------------------------------------------------------
    'FQ-R001': {
      id: 'FQ-R001',
      sportId: 'fitness',
  ...
  function recordChallengeResult(state, challengeId, score, arg4, arg5) {
    ...
    const sharePayload = {
      title: `KOINONIA Faith Quest — ${challenge.title}`,
      text: `KOINONIA Faith Quest — ${challenge.title}\nScore: ${scoreDisplay}${isPB ? '\n🌟 New Personal Best!' : ''}\n"A virtual world that grows when you grow in real life."`,
      challengeName: challenge.title,
      scoreText: scoreDisplay,
      isPersonalBest: isPB
    };

    // Reward Safety: Controlled first-time RPG rewards routed through KoinoniaRewardAuthority
    let rewardClaimed = false;
    let rewardDetails = null;

    // Resolve Reward Authority
    let rewardAuth = null;
    if (typeof KoinoniaRewardAuthority !== 'undefined') {
      rewardAuth = KoinoniaRewardAuthority;
    } else if (typeof global !== 'undefined' && global.KoinoniaRewardAuthority) {
      rewardAuth = global.KoinoniaRewardAuthority;
    } else if (typeof window !== 'undefined' && window.KoinoniaRewardAuthority) {
      rewardAuth = window.KoinoniaRewardAuthority;
    } else if (typeof require !== 'undefined') {
      try {
        rewardAuth = require('./quest_authority.js').KoinoniaRewardAuthority;
      } catch (_) {}
    }

    // Resolve Active Member ID
    let memberId = options.memberId || (options.context && options.context.memberId);
    if (!memberId && typeof BetaIdentityProvider !== 'undefined' && typeof BetaIdentityProvider.getCurrentIdentity === 'function') {
      const cur = BetaIdentityProvider.getCurrentIdentity();
      if (cur && (cur.memberId || cur.id)) memberId = cur.memberId || cur.id;
    } else if (!memberId && typeof BetaIdentity !== 'undefined' && typeof BetaIdentity.getCurrentIdentity === 'function') {
      const cur = BetaIdentity.getCurrentIdentity();
      if (cur && (cur.memberId || cur.id)) memberId = cur.memberId || cur.id;
    }
    if (!memberId && state && state.memberId) memberId = state.memberId;
    if (!memberId) memberId = 'youth_demo_01';

    let authorityResult = null;
    if (rewardAuth && typeof rewardAuth.recordChallengeReward === 'function') {
      authorityResult = rewardAuth.recordChallengeReward(memberId, challengeId, {
        sportId: actualSportId,
        score: Number(score)
      });
    }

    if (authorityResult && authorityResult.alreadyCompleted) {
      state.fitQuestRewardClaims[challengeId] = true;
    } else if (!state.fitQuestRewardClaims[challengeId]) {
      // First completion reward - use authority granted amounts
      let lpReward = 0;
      let xpReward = 0;

      if (authorityResult) {
        if (authorityResult.success && !authorityResult.alreadyCompleted) {
          lpReward = authorityResult.rewardsGranted.lifePoints;
          xpReward = authorityResult.rewardsGranted.xp;
        }
      } else {
        lpReward = challenge.rewards?.lifePoints || 5;
        xpReward = challenge.rewards?.charXp || 5;
      }

      // Authoritative progression sync from growth profile or reward authority (zero UI delta arithmetic)
      let growthSync = null;
      if (typeof KoinoniaGrowth !== 'undefined' && typeof KoinoniaGrowth.getGrowthProfile === 'function') {
        growthSync = KoinoniaGrowth.getGrowthProfile(memberId);
      } else if (typeof global !== 'undefined' && global.KoinoniaGrowth && typeof global.KoinoniaGrowth.getGrowthProfile === 'function') {
        growthSync = global.KoinoniaGrowth.getGrowthProfile(memberId);
      }
      if (growthSync && growthSync.progression) {
        state.lp = growthSync.progression.lifePoints;
        state.charXp = growthSync.progression.xp;
        if (typeof growthSync.progression.level === 'number') {
          state.charLevel = growthSync.progression.level;
        }
      } else if (rewardAuth && typeof rewardAuth.getRewardSummary === 'function') {
        const summary = rewardAuth.getRewardSummary(memberId);
        if (summary) {
  ...
  function createFitQuestReadyQaState(overrides = {}) {
    ...
    // Phase 0.18 / 0.20.1 Faith Quest & Sports State: STRICTLY EMPTY at QA baseline
