...
(function(global) {
  ...
  class PrototypeGrowthProvider extends BaseGrowthProvider {
    ...
    setRuntimeProfile(memberId, profile) {
      const mid = this._resolveMemberId(memberId);
      if (!mid) return;
      this.runtimeProfiles[mid] = profile;
      this.notifySubscribers(profile);
    }

    clearRuntimeProfile(memberId) {
      if (memberId) {
        const mid = this._resolveMemberId(memberId);
        if (mid) delete this.runtimeProfiles[mid];
      } else {
        this.runtimeProfiles = {};
      }
    }
    ...
    getGrowthProfile(identity) {
      ...
      if (this.runtimeProfiles[memberId]) {
        return this.runtimeProfiles[memberId];
      }
  ...
  const KoinoniaGrowth = {
    ...
    setRuntimeProfile(memberId, profile) {
      if (activeProvider && typeof activeProvider.setRuntimeProfile === 'function') {
        activeProvider.setRuntimeProfile(memberId, profile);
      }
    },
    clearRuntimeProfile(memberId) {
      if (activeProvider && typeof activeProvider.clearRuntimeProfile === 'function') {
        activeProvider.clearRuntimeProfile(memberId);
      }
    },
