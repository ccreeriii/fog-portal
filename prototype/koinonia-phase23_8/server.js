/**
 * KOINONIA — PHASE 0.23K SERVER
 * Secure Standalone Server with Dynamic Identity, Real-Time Presence & PWA Support
 *
 * Capabilities:
 * 1. Dedicated standalone HTTP & WSS server for staging/beta environments.
...
const mockMod = require("./test_support/mock_shared_core_server.js");
const MockSharedCoreServer = mockMod.MockSharedCoreServer || mockMod;
...
const DEFAULT_SERVICE_NAME = "koinonia";
...
function createBetaServer(options = {}) {
  ...
  const rawDevSharedCore = options.devSharedCore !== undefined
    ? options.devSharedCore
    : process.env.KOINONIA_DEV_SHARED_CORE;

  // Mock mode requires explicit configuration and is strictly rejected under NODE_ENV=production
  const isMockDev = !isProduction && (rawDevSharedCore === true || String(rawDevSharedCore).trim().toLowerCase() === "mock");

  const port = options.port !== undefined ? options.port : DEFAULT_PORT;
  const host = options.host || DEFAULT_HOST;
  const serviceName = (options.serviceName || process.env.KOINONIA_SERVICE_NAME || DEFAULT_SERVICE_NAME).trim();
  const environment = (options.environment || process.env.KOINONIA_ENVIRONMENT || (serviceName === "koinonia-staging" ? "staging" : (process.env.NODE_ENV === "production" ? "production" : (isProduction ? "production" : DEFAULT_ENVIRONMENT)))).trim();
  const mockServer = isMockDev ? new MockSharedCoreServer() : null;
  ...
  const httpServer = http.createServer((req, res) => {
    ...
    if (pathname === "/health" || pathname === "/api/health") {
      ...
      res.end(JSON.stringify({
        ...
        phase: PHASE,
        version: VERSION,
        sharedCoreMode: isMockDev ? "mock-readonly" : "local",
    ...
    // 1b. Server-Controlled Runtime Configuration Endpoint
    if (pathname === "/runtime-config.js") {
      ...
      const runtimeConfig = {
        ...
        phase: PHASE,
        version: VERSION,
        sharedCoreMode: isMockDev ? "mock" : "local",
        remoteReadsEnabled: isMockDev,
        remoteMutationsEnabled: false,
        baseUrl: ""
      };
      res.end(`window.__KOINONIA_RUNTIME_CONFIG__ = ${JSON.stringify(runtimeConfig, null, 2)};\n`);
      return;
    }

    // 1c. Same-Origin Shared Core Proxy (/api/v1/shared/*)
    if (pathname.startsWith("/api/v1/shared/")) {
      if (!isMockDev || !mockServer || !mockServer.port) {
        res.writeHead(404, {
          "Content-Type": "application/json; charset=utf-8",
          ...SECURITY_HEADERS
        });
        res.end(JSON.stringify({
          ...
          error: "NOT_FOUND",
          code: "NOT_FOUND",
          message: "Shared Core endpoint not found or mock proxy disabled."
        }));
        return;
      }
      ...
      if (pathname.includes("..")) {
        ...
        res.end(JSON.stringify({ success: false, error: "FORBIDDEN", message: "Path traversal rejected." }));
        return;
      }
      try {
        if (decodeURIComponent(pathname).includes("..")) {
          res.writeHead(403, { "Content-Type": "application/json; charset=utf-8", ...SECURITY_HEADERS });
          res.end(JSON.stringify({ success: false, error: "FORBIDDEN", message: "Path traversal rejected." }));
          return;
        }
        ...
        res.end(JSON.stringify({ success: false, error: "BAD_REQUEST", message: "Malformed path." }));
        return;
      }

      // Check against forbidden legacy routes
      ...
      for (const pattern of FORBIDDEN_ROUTE_PATTERNS) {
        if (pattern.test(pathname)) {
          ...
          res.end(JSON.stringify({ success: false, error: "NOT_FOUND", message: "Forbidden route." }));
          return;
        }
      }
      ...
      req.on("data", (chunk) => {
        ...
        if (bodyLength > MAX_HTTP_BODY_BYTES) {
          res.writeHead(413, { "Content-Type": "application/json", ...SECURITY_HEADERS });
          res.end(JSON.stringify({ success: false, error: "PAYLOAD_TOO_LARGE", message: "Payload Too Large" }));
          req.destroy();
        } else {
      ...
      req.on("end", () => {
        ...
        if (req.headers["idempotency-key"]) {
          forwardHeaders["Idempotency-Key"] = req.headers["idempotency-key"];
        }
        ...
        const forwardReq = http.request({
          ...
          const resChunks = [];
          forwardRes.on("data", (c) => resChunks.push(c));
          forwardRes.on("end", () => {
            const resBuffer = Buffer.concat(resChunks);
            const resHeaders = {
              "Content-Type": forwardRes.headers["content-type"] || "application/json; charset=utf-8",
              ...SECURITY_HEADERS
            };
            res.writeHead(forwardRes.statusCode, resHeaders);
            res.end(resBuffer);
          });
        });
        ...
        forwardReq.on("error", () => {
          if (!res.headersSent) {
            ...
            res.end(JSON.stringify({
              success: false,
              error: "BAD_GATEWAY",
              code: "SHARED_CORE_NETWORK_ERROR",
              message: "Mock Shared Core proxy connection failed."
            }));
          }
        });
        ...
        forwardReq.on("timeout", () => {
          forwardReq.destroy();
          if (!res.headersSent) {
            res.writeHead(504, { "Content-Type": "application/json; charset=utf-8", ...SECURITY_HEADERS });
            res.end(JSON.stringify({
              success: false,
              error: "GATEWAY_TIMEOUT",
              code: "SHARED_CORE_TIMEOUT",
              message: "Mock Shared Core proxy timed out."
            }));
          }
        });
  ...
  function close(callback) {
    ...
    wss.close(() => {
      httpServer.close(() => {
        if (mockServer && mockServerStarted) {
  ...
  function listen(cb) {
    if (mockServer && !mockServerStarted) {
      mockServer.start().then(() => {
        ...
        console.error("Failed to start mock Shared Core server:", err);
      });
    } else {
      httpServer.listen(port, host, () => {
        if (cb) cb(port, host);
...
if (require.main === module) {
  const port = parseInt(process.env.KOINONIA_DEV_PORT || process.env.PORT || DEFAULT_PORT, 10);
  const host = process.env.HOST || DEFAULT_HOST;
  const serviceName = (process.env.KOINONIA_SERVICE_NAME || DEFAULT_SERVICE_NAME).trim();
  const environment = (process.env.KOINONIA_ENVIRONMENT || (serviceName === "koinonia-staging" ? "staging" : (process.env.NODE_ENV === "production" ? "production" : DEFAULT_ENVIRONMENT))).trim();

  const server = createBetaServer({ port, host, serviceName, environment });
  server.listen((p, h) => {
    console.log("========================================================");
    console.log(`KOINONIA Phase ${PHASE} Server RUNNING (${serviceName} [${environment}])`);
    console.log(`HTTP URL:       http://${h}:${p}/`);
    console.log(`Health API:     http://${h}:${p}/health`);
    console.log(`Runtime Config: http://${h}:${p}/runtime-config.js`);
    console.log(`Shared Core:    ${server.devSharedCore ? "MOCK PROXY (READ-ONLY)" : "LOCAL (DEFAULT)"}`);
    if (server.mockServer && server.mockServer.port) {
      console.log(`Mock Upstream:  127.0.0.1:${server.mockServer.port} (ephemeral)`);
    }
