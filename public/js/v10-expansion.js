...
// FIRE OF GOD MINISTRIES - V11 GAME ENGINE (LANDING PAGES, 3-TRIES, SUDDEN DEATH, AUTO-REFRESH)
...
window.V10Expansion = {
    ...
    // --------------------------------------------------------------------------
    // TOP 3 SCORERS AUTO-REFRESH ENGINE
    // --------------------------------------------------------------------------
    refreshTopScorers: async function(btnElem) {
        if(btnElem) {
            const origHTML = btnElem.innerHTML;
            btnElem.innerHTML = '⏳';
            await this.loadTopScorers(true); // Force clear cache and refresh
            btnElem.innerHTML = origHTML;
        } else {
            await this.loadTopScorers(true);
        }
    },

    loadTopScorers: async function(force = false) {
        try {
            if (force || !this.bulkDataCache || Object.keys(this.bulkDataCache).length === 0) {
                const res = await fetch('/api/games/top-scorers-bulk');
                this.bulkDataCache = await res.json();
            }
            ...
            if(force) this.loadFeaturedGames(); // Re-render heroes too!
    ...
    openGameLanding: async function(gameName, type, playFn, desc, icon) {
        ...
        try {
            const topPlayers = this.bulkDataCache[gameName] || [];
            const attRes = await fetch(`/api/games/check-attempts?youth_id=${currentMember.id}&game_name=${encodeURIComponent(gameName)}`);
            ...
            const returnScript = type === 'growth' 
                ? `document.getElementById('growthActiveGameArea').style.display='none'; document.getElementById('growthGamesGrid').style.display='grid'; if(document.getElementById('featuredGrowthGameContainer')) document.getElementById('featuredGrowthGameContainer').style.display='block';` 
    ...
    submitUniversalScore: async function(gameName, type, score) {
        try {
            ...
            if (data.success) {
                ...
                // 🔄 AUTO-REFRESH: Clear cache and pre-fetch updated scores silently
                this.bulkDataCache = {};
                this.loadTopScorers(true);
    ...
    playCC: async function() {
        ...
        this._activeTimer = setInterval(() => {
            ...
            if(this.ccState.t <= 0) { clearInterval(this._activeTimer); this.submitUniversalScore("Catechism Clash", "growth", this.ccState.s, 'growthActiveGameArea'); }
        }, 1000);
        this.renderCC();
    },
    renderCC: function() {
        if(this.ccState.i >= this.ccState.q.length) { clearInterval(this._activeTimer); return this.submitUniversalScore("Catechism Clash", "growth", this.ccState.s, 'growthActiveGameArea'); }
    ...
    ansCC: function(sel, cor, btn) {
        if(sel === cor){ 
            ...
            setTimeout(() => { this.submitUniversalScore("Catechism Clash", "growth", this.ccState.s, 'growthActiveGameArea'); }, 1000);
        }
    },
    ...
    renderWAI: function() {
        if(this.waiState.i >= this.waiState.q.length) return this.submitUniversalScore("Who Am I?", "growth", this.waiState.s, 'growthActiveGameArea');
    ...
    ansWAI: function(ans, pts) {
        ...
        if(guess === ans.toLowerCase()) { 
            ...
            setTimeout(() => { this.submitUniversalScore("Who Am I?", "growth", this.waiState.s, 'growthActiveGameArea'); }, 1500);
        }
    },
    ...
    setupVS: function() {
        if(this.vsState.i >= this.vsState.q.length) return this.submitUniversalScore("Daily Manna Scramble", "growth", this.vsState.s, 'growthActiveGameArea');
    ...
    ansVS: function() {
        ...
        if(this.vsState.cur.join(' ') === this.vsState.correctOrder.join(' ')) {
            ...
            setTimeout(() => { this.submitUniversalScore("Daily Manna Scramble", "growth", this.vsState.s, 'growthActiveGameArea'); }, 1500);
        }
    },
    ...
    renderEM: function() {
        if(this.emState.i >= this.emState.q.length) return this.submitUniversalScore("Emoji Sermon Translator", "growth", this.emState.s, 'growthActiveGameArea');
    ...
    ansEM: function(guess, ans, btn) {
        if(guess === ans) { 
            ...
            setTimeout(() => { this.submitUniversalScore("Emoji Sermon Translator", "growth", this.emState.s, 'growthActiveGameArea'); }, 1000);
        }
    },
    ...
    renderNG: function() {
        if(this.ngState.i >= this.ngState.q.length) return this.submitUniversalScore("The Narrow Gate", "growth", this.ngState.s, 'growthActiveGameArea');
    ...
    ansNG: function(sel, cor, btn) {
        if(sel === cor) { 
            ...
            setTimeout(() => { this.submitUniversalScore("The Narrow Gate", "growth", this.ngState.s, 'growthActiveGameArea'); }, 800);
        }
    },
    ...
    startRXRound: function() {
        if(this.rxState.i >= 15) return this.submitUniversalScore("Shield of Faith: Reflex Tap", "growth", this.rxState.s, 'growthActiveGameArea');
    ...
    ansRX: function(ans, btn) {
        ...
        } else this.submitUniversalScore("Shield of Faith: Reflex Tap", "growth", this.rxState.s, 'growthActiveGameArea');
    },
