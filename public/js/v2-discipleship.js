        ...
        try {
            ...
            if (userList) {
                ...
                userList.innerHTML = this.groupsData.map(g => {
                    ...
                    return `<div style="background:#FFF; border: 1px solid var(--border-color); padding: 15px; margin-bottom: 15px; border-radius: 8px; display:flex; gap:15px; align-items:center;">${logoHtml}<div style="flex:1;"><h3 style="color:var(--primary); margin-bottom:5px;">${g.name}</h3><p style="font-size:0.9rem; color:var(--text-muted); margin-bottom:12px;">📅 ${g.meeting_schedule || 'TBA'} <br>📍 ${g.venue || 'TBA'} <br>👤 Leader: ${g.leader_name || 'TBA'}</p><div style="display: flex; gap: 10px;"><button class="btn btn-outline btn-sm" style="flex: 1; border-color:var(--primary); color:var(--primary);" onclick="V2Discipleship.joinSmallGroup(${g.id})">Join</button><button class="btn btn-outline btn-sm" style="flex: 1; border-color: #F59E0B; background: #FEF3C7; color: #D97706; font-weight: bold;" onclick="openGroupSpace(${g.id}, '${g.name.replace(/'/g, "\\'")}')">🔥 Chat</button><button class="btn btn-outline btn-sm" style="flex: 1; border-color: #3B82F6; background: #EFF6FF; color: #2563EB; font-weight: bold;" onclick="openGroupVault(${g.id}, '${g.name.replace(/'/g, "\\'")}')">🎥 Vault</button></div></div></div>`;
                }).join('');
            }
        } catch(e) {}
    },
