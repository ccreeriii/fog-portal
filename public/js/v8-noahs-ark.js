window.V8NoahsArk = {
    ...
    // 🛡️ FIX: Hooks directly into the Universal Scoring Engine
    handleGameOver: function(titleText = "Game Over!") {
        this.isPlaying = false;
        clearInterval(this.timerInterval);

        // V10 Expansion engine will take over the UI and overwrite the game container
        if (typeof window.V10Expansion !== 'undefined' && typeof window.V10Expansion.submitUniversalScore === 'function') {
            window.V10Expansion.submitUniversalScore("Noah's Ark: Rescue", "arcade", this.score);
        } else {
            alert(titleText + " Score: " + this.score);
            this.exitGame();
        }
