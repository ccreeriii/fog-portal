window.V8NoahsArk = {
        ...
        area.innerHTML = `
            ...
            <div style="position: relative; display: flex; flex-direction: column; align-items: center; background: #FFFFFF; padding: 20px 0;">
                ...
                >
                    <div
                        id="naDpad"
                        aria-label="Noah movement controls"
                        style="
                            display: grid;
                    ...
                    >
                        ...
                        <button
                            ...
                            aria-label="Move up"
                            style="width:68px;height:68px;padding:0;font-size:1.8rem;border-radius:14px;background:#F8FAFC;"
                            onclick="V8NoahsArk.move(0, -1)"
                        >⬆️</button>
                        ...
                        <button
                            ...
                            style="width:68px;height:68px;padding:0;font-size:1.8rem;border-radius:14px;background:#F8FAFC;"
                            onclick="V8NoahsArk.move(-1, 0)"
                        >⬅️</button>

                        <div
                            aria-hidden="true"
                            style="width:68px;height:68px;border-radius:14px;background:#F1F5F9;border:1px dashed #CBD5E1;"
                        ></div>
                    ...
                    <button
                        ...
                        onclick="V8NoahsArk.restartCurrentLevel()"
                        style="
                            flex:0 0 92px;
                            min-height:56px;
                            padding:8px;
    ...
    restartCurrentLevel: function() {
        ...
        const confirmed =
            window.confirm(
                'Restart this level? Your current level and score will be kept.'
            );

        if (!confirmed) return;

        /*
         * Restart only the current board.
         * Score and level remain part of the same run.
         */
        this.startLevel();
    },
    ...
    handleLevelWin: function() {
        ...
        if (this.level >= this.maxLevel) {
            overlay.innerHTML = `
                ...
                <button class="btn btn-primary" onclick="V8NoahsArk.handleGameOver('Victory!')">Save Score & Exit</button>
            `;
        } else {
            overlay.innerHTML = `
    ...
    handleGameOver: async function(titleText = "Game Over!") {
        ...
        await window.V10Expansion.submitCanvasGameResult({ gameName: "Noah's Ark: Rescue", score: finalScore, overlayId: 'naOverlay', playAgain: 'V8NoahsArk.startGame()' });
        this.score = 0;
    }
};
